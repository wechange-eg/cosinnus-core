    ...
    def get_my_stream_for_user(self, user, portals=''):
        ...
        if stream is None:
            ...
            try:
                ...
                stream = self.model._default_manager.create(
                    creator=user,
                    title='_my_stream_%s' % portal_str,
                    slug='_my_stream_%s' % portal_str,
                    is_my_stream=True,
                    portals=portals,
                )
...
class Stream(StreamManagerMixin, BaseTaggableObjectModel):
    ...
    portals = django_models.CharField(
        _('Portal IDs'),
        blank=True,
        null=False,
        max_length=255,
    ...
    special_groups = django_models.CharField(
        _('Special Group IDs'),
        blank=True,
        null=False,
        max_length=255,
    ...
    def update_cache(self, user):
        try:
            cache.set(
                USER_STREAM_SHORT_CACHE_KEY % (self.portals, user.id if user.is_authenticated else 0),
                self,
                settings.COSINNUS_STREAM_SHORT_CACHE_TIMEOUT,
            )
        except Exception as e:
            # sometimes we cannot pickle the deep cache and it throws errors, we don't want to let this bubble up
            logger.warning(
                'Could not save the user stream into the cache because of an exception! (in extra)',
...
post_save.connect(
    create_special_streams_profile,
