...
class StreamManagerMixin(object):
    def _get_querysets_for_stream(self, stream, user, include_public=False):
        ...
        for registered_model in aor:
            ...
            if (
                registered_model.lower() in settings.COSINNUS_REFLECTABLE_OBJECTS
                and BaseTaggableObjectModel in inspect.getmro(model_class)
            ):
    ...
    def _get_stream_objectset(self, querysets):
        ...
        for queryset in querysets:
            ...
            setlist[queryset.model.__name__] = {'qs': queryset, 'objs': [], 'offset': start}
    ...
    def _filter_queryset_for_stream(self, queryset, stream, user, include_public=False):
        ...
        if stream.is_my_stream:
            ...
            if user.is_authenticated:
                self.stream_user_group_ids = getattr(
                    self, 'user_group_ids', get_cosinnus_group_model().objects.get_for_user_pks(user)
                )
            ...
            if stream.media_tag:
                ...
                if stream.media_tag.location_lat and stream.media_tag.location_lon:
                    ...
                    queryset = queryset.filter(
                        media_tag__location_lat__range=lat_range, media_tag__location_lon__range=lon_range
                    )
