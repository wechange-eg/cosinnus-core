...
from .forms import AnonymousWriteForm, FullReplyForm, QuickReplyForm, WriteForm
    ...
    def check_restricted_recipient(self, message, user=None):
        if message.multi_conversation:
            ...
            if (
                ...
                and getattr(settings, 'NEWW_FORUM_GROUP_SLUG') in recipient_group_slugs
                and not check_user_superuser(user or self.request.user)
            ):
                return True
        return False
                ...
                users = user_model.objects.filter(
                    ...
                    **{
                        '{0}__in'.format(name_user_as): [
                            r.strip() for r in user_recipients.split(',') if r and not r.isspace()
                        ]
                    },
                ).order_by(name_user_as)
                usernames = [
                    'user:%s' % getattr(user, name_user_as)
                    for user in users
                    if check_user_can_see_user(self.request.user, user) and not user.id == self.request.user.id
                ]
                if usernames:
    ...
    def get_form_kwargs(self):
        ...
        if self.request.method == 'POST':
            ...
            if self.parent.multi_conversation:
                ...
                post = kwargs['data'].copy()  # self.request.POST is immutable
                post.setlist(
                    'recipients', ['user:%d' % rec.id for rec in self.parent.multi_conversation.participants.all()]
                )
                kwargs['data'] = post
        return kwargs
        ...
        context.update(
            {
                ...
                or reverse('postman:inbox', current_app=self.request.resolver_match.namespace),
