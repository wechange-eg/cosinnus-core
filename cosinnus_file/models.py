...
from cosinnus.models.tagged import BaseHierarchicalTaggableObjectModel, LikeableObjectMixin
...
def get_hashed_filename(instance, filename):
    ...
    path = join(
        ...
        'files',
        force_str(instance.group_id),
        force_str(time.year),
        force_str(time.month),
    )
...
class FileEntry(
    ThumbnailableImageMixin, VideoEmbedFieldMixin, LikeableObjectMixin, BaseHierarchicalTaggableObjectModel
):
    """
    ...
    file = models.FileField(_('File'), blank=True, null=True, max_length=250, upload_to=get_hashed_filename)
...
def get_or_create_attachment_folder(group):
    ...
    try:
        attachment_folder = FileEntry.objects.get(is_container=True, group=group, special_type='attached')
    except FileEntry.DoesNotExist:
        pgettext('special_folder_type', 'Attachments')  # leave this for i18n to find!
        attachment_folder = FileEntry(title='Attachments', is_container=True, group=group, special_type='attached')
