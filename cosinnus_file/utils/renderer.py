...
class FileEntryRenderer(HierarchicalListCreateViewMixin, BaseRenderer):
    ...
    def render(cls, context, myobjs, **kwargs):
        ...
        return super(FileEntryRenderer, cls).render(
            context, files=files, images=images, all_files=files + images, **kwargs
        )
    ...
    def render_list_for_user(cls, user, request, qs_filter={}, limit=30, render_if_empty=True, **kwargs):
        ...
        if not render_if_empty and (
            not renderer.object_list or len(renderer.object_list) == 1 and renderer.object_list[0].is_container
        ):
            return None
