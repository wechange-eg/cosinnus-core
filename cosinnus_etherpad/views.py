...
class EtherpadIndexView(RequireReadMixin, RedirectView):
    ...
    def get_redirect_url(self, **kwargs):
        return group_aware_reverse('cosinnus:etherpad:list', kwargs={'group': self.group})


index_view = EtherpadIndexView.as_view()


class EtherpadListView(RequireReadMixin, FilterGroupMixin, HierarchyTreeMixin, ListView):
    model = Etherpad
...
class EtherpadDetailView(RequireReadMixin, RecordLastVisitedMixin, FilterGroupMixin, DetailView):
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        response = super(EtherpadDetailView, self).render_to_response(context, **response_kwargs)
        ...
        try:
            ...
            if domain:
                ...
                if domain not in server_name and server_name not in domain and server_name != '127.0.0.1':
                    ...
                    logging.warning(
                        "SERVER_NAME %s and cookie domain %s don't match. Setting a third-party cookie might not work!"
                        % (server_name, domain)
                    )
            response.set_cookie('sessionID', user_session_id, domain=domain)
        except (HTTPError, EtherpadException, URLError) as exc:
            logger.error(
                'Cosinnus Etherpad DetailView configuration error: Etherpad error',
                extra={'exception': exc, 'url': self.request.META.get('HTTP_REFERER', 'N/A')},
            )
            messages.error(
                self.request,
                _(
...
class EtherpadWriteView(RequireLoggedInMixin, EtherpadDetailView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if (
            ...
            and not settings.COSINNUS_USER_GUEST_ACCOUNTS_ENABLE_SOFT_EDITS
        ):
...
class EtherpadFormMixin(FilterGroupMixin, GroupFormKwargsMixin, UserFormKwargsMixin):
...
class EtherpadHybridListView(
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            if 'padName does already exist' in force_str(exc):
                ...
                logger.error(
                    'Cosinnus Etherpad ListView configuration error: Etherpad Misconfigured',
                    extra={'exception': exc, 'url': self.request.META.get('HTTP_REFERER', 'N/A')},
                )
                messages.error(
                    self.request,
            ...
            logger.error(
                'Cosinnus Etherpad ListView configuration error: Etherpad URL invalid',
                extra={'exception': exc, 'url': self.request.META.get('HTTP_REFERER', 'N/A')},
            )
            messages.error(
                self.request,
    ...
    def get_success_url(self):
        if self.object.is_container:
            ...
            return group_aware_reverse('cosinnus:etherpad:list', kwargs={'group': self.group, 'slug': self.object.slug})
        else:
            return group_aware_reverse(
                'cosinnus:etherpad:pad-edit', kwargs={'group': self.group, 'slug': self.object.slug}
            )
...
class EtherpadEditView(RequireWriteMixin, EtherpadFormMixin, AttachableViewMixin, UpdateView):
    ...
    def render_to_response(self, context, **response_kwargs):
        response = super(EtherpadFormMixin, self).render_to_response(context, **response_kwargs)
        ...
        try:
            ...
            logger.error(
                'Cosinnus Etherpad EditView configuration error: Etherpad error',
                extra={'exception': exc, 'url': self.request.META.get('HTTP_REFERER', 'N/A')},
            )
            messages.error(
                self.request,
...
class EthercalcDownloadBaseView(RequireReadMixin, FilterGroupMixin, DetailView):
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        if not resp.status_code == 200:
            ...
            return redirect(
                group_aware_reverse('cosinnus:etherpad:pad-write', kwargs={'group': calc.group, 'slug': calc.slug})
            )
    ...
    def post(self, request, *args, **kwargs):
        if 'cosinnus_document' in settings.INSTALLED_APPS:
            ...
            try:
                ...
                doc = Document(title=title, group=self.group, created=request.user, is_draft=False)
            doc.content = pad.content
            doc.save()

            msg = _('Pad has been archived as Document: <a class="alert-link" href="%(href)s">%(title)s</a>') % {
                'href': group_aware_reverse(
                    'cosinnus:document:document-detail',
                    kwargs={
                        'group': self.group,
                        'slug': doc.slug,
                    },
...
class EtherpadArchiveFileView(EtherpadArchiveMixin):
    def _create_folder(self, request, path):
        ...
        try:  # don't use get_or_create: creator doesn't matter for get
            ...
            FileEntry.objects.create(title=title, group=self.group, isfolder=True, creator=request.user, path=path)
    ...
    def post(self, request, *args, **kwargs):
        if 'cosinnus_file' in settings.INSTALLED_APPS:
            ...
            try:
                ...
                entry = FileEntry(title=title, group=self.group, creator=request.user, mimetype='text/html', path=path)
