...
from cosinnus.utils.files import (
    ...
    get_organization_avatar_filename,
    get_organization_wallpaper_filename,
    image_thumbnail,
    image_thumbnail_url,
)
...
class OrganizationManager(models.Manager):
    ...
    def get_pks(self, portal_id=None, force=True):
        ...
        if force or pks is None:
            ...
            cache.set(
                self._ORGANIZATIONS_PK_TO_SLUG_CACHE_KEY % (portal_id),
                pks,
...
class CosinnusOrganizationMembership(BaseMembership):
    group = models.ForeignKey(
        'cosinnus_organization.CosinnusOrganization', related_name='memberships', on_delete=models.CASCADE
    )
...
class CosinnusUnregisteredUserOrganizationInvite(BaseMembership):
    ...
    group = models.ForeignKey(
        'cosinnus_organization.CosinnusOrganization', related_name='unregistered_user_invites', on_delete=models.CASCADE
    )
    ...
    invited_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name='cosinnus_organization_invitations',
...
class CosinnusOrganization(IndexingUtilsMixin, MembersManagerMixin, models.Model):
    ...
    avatar = models.ImageField(
        _('Logo'), null=True, blank=True, upload_to=get_organization_avatar_filename, max_length=250
    )
    ...
    users = models.ManyToManyField(
        settings.AUTH_USER_MODEL,
        blank=True,
        related_name='cosinnus_organizations',
        through=CosinnusOrganizationMembership,
    )
    ...
    def _clear_cache(self, slug=None, slugs=None):
        ...
        if slugs:
            keys.extend(
                [self.objects._ORGANIZATIONS_SLUG_CACHE_KEY % (CosinnusPortal.get_current().id, s) for s in slugs]
            )
...
class CosinnusOrganizationGroup(models.Model):
    organization = models.ForeignKey(
        'cosinnus_organization.CosinnusOrganization', related_name='groups', on_delete=models.CASCADE
    )
    group = models.ForeignKey(
        settings.COSINNUS_GROUP_OBJECT_MODEL, related_name='organizations', on_delete=models.CASCADE
    )
    ...
    def save(self, *args, **kwargs):
        ...
        # or admin
        if (
            self._old_current_status == MEMBERSHIP_PENDING or self._old_current_status == MEMBERSHIP_INVITED_PENDING
        ) and (self.status in MEMBER_STATUS):
            self.date = now()
