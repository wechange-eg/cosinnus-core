...
from cosinnus_organization.forms import (
    CosinnusOrganizationForm,
    CosinnusOrganizationLocationInlineFormset,
...
class CosinnusOrganizationFormMixin(object):
    ...
    def dispatch(self, *args, **kwargs):
        ...
        if (
            getattr(settings, 'COSINNUS_LIMIT_ORGANIZATION_CREATION_TO_ADMINS', False)
        ...
        if not getattr(settings, 'COSINNUS_USERS_CAN_CREATE_ORGANIZATIONS', False) and self.form_view == 'add':
...
class OrganizationCreateView(
    RequireLoggedInMixin, CosinnusOrganizationFormMixin, AvatarFormMixin, AdditionalFormsMixin, CreateWithInlinesView
):
    ...
    def get_success_url(self):
        ...
        if len(self.extra_forms) > 0:
            ...
            if tab < len(self.extra_forms):
                return (
                    reverse('cosinnus:organization-edit', kwargs={'organization': self.object.slug})
                    + f'./?tab={tab + 1}'
                )
    ...
    def forms_valid(self, form, inlines):
        ...
        membership = CosinnusOrganizationMembership.objects.create(
        ...
        cosinnus_notifications.organization_created.send(
            sender=self, user=self.request.user, obj=self.object, audience=[]
        )
...
class OrganizationEditView(
    RequireWriteGrouplessMixin,
    AvatarFormMixin,
    CosinnusOrganizationFormMixin,
...
class OrganizationDeleteView(RequireWriteGrouplessMixin, SamePortalGroupMixin, AjaxFormsDeleteViewMixin, DeleteView):
    model = CosinnusOrganization
    slug_url_kwarg = 'organization'
