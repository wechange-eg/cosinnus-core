...
class BaseTagObject(models.Model):
    ...
    VISIBILITY_CHOICES = (('', ''),) + _VISIBILITY_CHOICES
    VISIBILITY_VALID_VALUES = (VISIBILITY_USER, VISIBILITY_GROUP, VISIBILITY_ALL)
    ...
    persons = models.ManyToManyField(settings.AUTH_USER_MODEL, blank=True, verbose_name=_('Persons'), related_name='+')
    ...
    place = models.CharField(_('Place'), max_length=100, default='', blank=True)
    ...
    approach = models.CharField(_('Approach'), blank=True, null=True, choices=APPROACH_CHOICES, max_length=255)
    ...
    topics = models.CharField(
        _('Topics'), blank=True, null=True, max_length=255, validators=[validate_comma_separated_integer_list]
    )

    text_topics = models.ManyToManyField(
        CosinnusTopicCategory, verbose_name=_('Text Topics'), related_name='tagged_objects', blank=True
    )
    ...
    )  # no reverse relation on model

    bbb_room = models.ForeignKey(
        'cosinnus.BBBRoom',
        null=True,
        blank=True,
...
class AttachableObjectModel(models.Model):
    ...
    def attached_image(self):
        ...
        for attached_file in self.attached_objects.all():
            if (
                attached_file.model_name == 'cosinnus_file.FileEntry'
                and attached_file.target_object is not None
                and attached_file.target_object.is_image
            ):
...
class LastVisitedObject(models.Model):
    ...
    visited = models.DateTimeField(verbose_name=_('Visited'), auto_now_add=True)
...
class LastVisitedMixin(object):
    ...
    def mark_visited(self, user):
        ...
        try:
            ...
            visit = get_object_or_None(
                LastVisitedObject, content_type=ct, object_id=self.id, user=user, portal=CosinnusPortal.get_current()
            )
            if visit is None:
                visit = LastVisitedObject(
                    content_type=ct, object_id=self.id, user=user, portal=CosinnusPortal.get_current()
                )
            ...
            logger.exception(
                'An unknown error occured while saving the last_visited visit! Exception in extra.',
...
class BaseTaggableObjectModel(LastVisitedMixin, IndexingUtilsMixin, AttachableObjectModel):
    ...
    media_tag = models.OneToOneField(
        settings.COSINNUS_TAG_OBJECT_MODEL, blank=True, null=True, on_delete=models.SET_NULL
    )
    ...
    def on_save_added_tagged_persons(self, set_users):
        ...
        cosinnus_notifications.user_tagged_in_object.send(
            sender=self, user=self.creator, obj=self, audience=list(set_users)
        )
    ...
    def render_additional_notification_content_rows(self):
        ...
        if tag_object.location and tag_object.location_url:
            ...
            location_html = render_to_string(
                'cosinnus/html_mail/content_snippets/tagged_location.html',
...
class BaseHierarchicalTaggableObjectModel(BaseTaggableObjectModel):
    ...
    is_container = models.BooleanField(blank=False, null=False, default=False, editable=True)
    ...
    path = models.CharField(_('Path'), blank=False, null=False, default='/', max_length=250)
    ...
    def container(self):
        ...
        while BaseHierarchicalTaggableObjectModel not in folder_class.__bases__:
...
class BaseTaggableObjectReflection(models.Model):
    ...
    group = models.ForeignKey(
        settings.COSINNUS_GROUP_OBJECT_MODEL, on_delete=models.CASCADE, related_name='reflected_objects'
    )

    creator = models.ForeignKey(
        settings.AUTH_USER_MODEL, verbose_name=_('Creator'), on_delete=models.CASCADE, related_name='+'
    )
...
class LikeObject(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, verbose_name=_('User'), on_delete=models.CASCADE, null=True, related_name='likes'
    )
...
class LikeableObjectMixin(models.Model):
    ...
    _LIKED_OBJECT_USER_IDS_CACHE_KEY = (
        'cosinnus/core/like_user_ids/model/%s/obj_id/%d'  # modelclass_name, id -> [user_id, user_id, ...]
    )
    # key storing all user ids that are following an object
    _FOLLOWED_OBJECT_USER_IDS_CACHE_KEY = (
        'cosinnus/core/follow_user_ids/model/%s/obj_id/%d'  # modelclass_name, id -> [user_id, user_id, ...]
    )
    # key storing all user ids that are starring an object
    _STARRED_OBJECT_USER_IDS_CACHE_KEY = (
        'cosinnus/core/star_user_ids/model/%s/obj_id/%d'  # modelclass_name, id -> [user_id, user_id, ...]
    )
    # local caches
    _liked_obj_ids = None
    ...
    def get_liked_user_ids(self):
        ...
        if user_ids is None:
            ...
            cache.set(
                ...
                user_ids,
                settings.COSINNUS_LIKEFOLLOW_COUNT_CACHE_TIMEOUT,
            )
...
def get_tag_object_model():
    ...
    if tag_model is None:
        raise ImproperlyConfigured(
            "COSINNUS_TAG_OBJECT_MODEL refers to model '%s' that has not been installed"
