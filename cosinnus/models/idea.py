    ...
    def annotate_likes(self):
        ...
        ann = self.annotate(
            like_count=models.Count(
                models.Case(models.When(likes__liked=True, then=1), default=0, output_field=models.IntegerField())
            )
        )
        return ann
...
class IdeaManager(models.Manager):
    ...
    def get_pks(self, portal_id=None, force=False):
        ...
        if force or pks is None:
            ...
            cache.set(self._IDEAS_PK_TO_SLUG_CACHE_KEY % (portal_id), pks, settings.COSINNUS_IDEA_CACHE_TIMEOUT)
        return pks
...
class CosinnusIdea(IndexingUtilsMixin, LikeableObjectMixin, models.Model):
    ...
    creator = models.ForeignKey(
        settings.AUTH_USER_MODEL, verbose_name=_('Creator'), on_delete=models.CASCADE, null=True, related_name='ideas'
    )
    ...
    created_groups = models.ManyToManyField(
        settings.COSINNUS_GROUP_OBJECT_MODEL, verbose_name=_('Created Projects'), blank=True, related_name='+'
    )
        ...
        if created:
            ...
            if forum_slug:
                ...
                if forum_group:
                    ...
                    signals.idea_object_created.send(sender=self, group=forum_group)
    ...
    def get_absolute_url(self):
        ...
        return get_domain_for_portal(self.portal_id) + reverse('cosinnus:map') + '?item=' + item_id
