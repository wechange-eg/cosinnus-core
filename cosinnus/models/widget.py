...
class WidgetConfig(models.Model):
    ...
    type = models.PositiveIntegerField(
        _('Widget Type'), choices=TYPE_CHOICES, default=TYPE_DASHBOARD, editable=False, null=False, blank=False
    )
    ...
    def __str__(self):
        return '{0} - {1} - {2}'.format(
            self.app_name, self.widget_name, 'group %s' % self.group if self.group else 'user %s' % self.user
        )
