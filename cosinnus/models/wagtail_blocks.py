...
from wagtail.core.blocks.field_block import RawHTMLBlock, RichTextBlock
...
class BetterDbWhitelister(DbWhitelister):
    ...
    def clean_tag_node(cls, doc, tag):
        if 'data-embedtype' not in tag.attrs and tag.name == 'div':
...
class MapBlock(StructBlock):
    ...
    def clean(self, value):
        ...
        if not any([bool(value.get(val)) for val in ('people', 'events', 'projects', 'groups')]):
            errors = dict(
                [
                    (val, ErrorList([_('At least one type of Map object must be chosen!')]))
