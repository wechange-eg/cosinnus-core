...
class BaseMembershipManager(models.Manager):
    ...
    def get_invited_pendings(self, group=None, groups=None):
        ...
        if group:
            ...
            gids = [isinstance(g, int) and g or g.pk for g in groups]
            return self._get_users_for_multiple_groups(
                gids, _MEMBERSHIP_INVITED_PENDINGS_KEY, MEMBERSHIP_INVITED_PENDING
            )
    ...
    def get_managers(self, group=None, groups=None):
        ...
        if group:
            ...
            gids = [isinstance(g, int) and g or g.pk for g in groups]
            return self._get_users_for_multiple_groups(gids, _MEMBERSHIP_MANAGERS_KEY, MEMBERSHIP_MANAGER)
...
class BaseMembership(models.Model):
    ...
    def __init__(self, *args, **kwargs):
        if not self.CACHE_KEY_MODEL:
            raise ImproperlyConfigured(
                'You must define a cache key specific to '
    ...
    def save(self, *args, **kwargs):
        ...
        if hasattr(self, 'user'):
            ...
            if not hasattr(self.user, 'cosinnus_profile') or not self.user.cosinnus_profile:
    ...
    def _refresh_cache(self):
        self.clear_member_cache_for_group(self.group)
        type(self).objects._get_users_for_single_group(
            self.group.id, _MEMBERSHIP_MEMBERS_KEY, MEMBER_STATUS, clear_cache=True
        )
        type(self).objects._get_users_for_single_group(
            self.group.id, _MEMBERSHIP_ADMINS_KEY, MEMBERSHIP_ADMIN, clear_cache=True
        )
        type(self).objects._get_users_for_single_group(
            self.group.id, _MEMBERSHIP_PENDINGS_KEY, MEMBERSHIP_PENDING, clear_cache=True
        )
        type(self).objects._get_users_for_single_group(
            self.group.id, _MEMBERSHIP_INVITED_PENDINGS_KEY, MEMBERSHIP_INVITED_PENDING, clear_cache=True
        )
        type(self).objects._get_users_for_single_group(
            self.group.id, _MEMBERSHIP_MANAGERS_KEY, MEMBERSHIP_MANAGER, clear_cache=True
        )
