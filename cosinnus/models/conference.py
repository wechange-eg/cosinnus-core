...
class CosinnusConferenceSettings(models.Model):
    ...
    BBB_SERVER_CHOICES_WITH_INHERIT = tuple(list(settings.COSINNUS_BBB_SERVER_CHOICES) + [CHOICE_INHERIT])
    ...
    bbb_server_choice = models.PositiveSmallIntegerField(
        _('BBB Server'),
        blank=False,
    ...
    def get_for_object(cls, source_object, no_traversal=False, recursed=False):
        ...
        if not setting_obj:
            ...
            # from now on, setting_obj is either None or of type cosinnus.CosinnusConferenceSettings
            ...
            if not recursed:
                # set bbb room nature
                if setting_obj and hasattr(source_object, 'get_bbb_room_nature'):
    ...
    def set_bbb_preset_form_field_values(self, preset_choices_dict):
        ...
        for field_name, choice_value in preset_choices_dict.items():
            ...
            if (
                field_name in settings.BBB_PRESET_FORM_FIELD_PARAMS
                and choice_value is not None
                and choice_value != self.SETTING_INHERIT
            ):
...
class CosinnusConferenceRoomManager(models.Manager):
    ...
    def get_queryset(self):
        return (
            CosinnusConferenceRoomQS(self.model, using=self._db).select_related('group').order_by('sort_index', 'title')
        )


@six.python_2_unicode_compatible
class CosinnusConferenceRoom(
    TranslateableFieldsModelMixin, BBBRoomMixin, ModelInheritsGroupReadWritePermissionsMixin, models.Model
):
    ...
    group = models.ForeignKey(
        settings.COSINNUS_GROUP_OBJECT_MODEL, verbose_name=_('Team'), related_name='rooms', on_delete=models.CASCADE
    )
    ...
    type = models.PositiveSmallIntegerField(
        _('Conference Room Type'), blank=False, default=TYPE_LOBBY, choices=TYPE_CHOICES
    )

    is_visible = models.BooleanField(
        _('Is visible'), help_text='If a room is not visible, it is not shown in any room lists', default=True
    )
    sort_index = models.PositiveSmallIntegerField(
        _('Sorting index'), help_text='Rooms are ordered in ascending order on this field', default=1
    )

    created = models.DateTimeField(verbose_name=_('Created'), editable=False, auto_now_add=True)
    creator = models.ForeignKey(
        settings.AUTH_USER_MODEL, verbose_name=_('Creator'), on_delete=models.CASCADE, null=True, related_name='rooms'
    )
    ...
    rocket_chat_room_name = models.CharField(
        _('RocketChat room name'),
        max_length=250,
        null=True,
        blank=True,
    ...
    show_chat = models.BooleanField(_('Show chat'), help_text='Show rocket chat in sidebar', default=True)

    # Type: CoffeeTable field only
    allow_user_table_creation = models.BooleanField(
        _('Allow users to create new coffee tables'),
    ...
    # Type: CoffeeTable field only
    max_coffeetable_participants = models.PositiveSmallIntegerField(
        _('Maximum Coffee Table Participants'),
        blank=False,
    ...
    target_result_group = models.OneToOneField(
        settings.COSINNUS_GROUP_OBJECT_MODEL,
        verbose_name=_('Result Project'),
    ...
    def get_maintenance_url(self):
        return group_aware_reverse(
            'cosinnus:conference:page-maintenance-room', kwargs={'group': self.group, 'slug': self.slug}
        )
    ...
    def get_room_create_url(self):
        return group_aware_reverse(
            'cosinnus:event:conference-event-add', kwargs={'group': self.group, 'room_slug': self.slug}
        )

    def get_rocketchat_room_url(self):
        if (
            not settings.COSINNUS_ROCKET_ENABLED
            or self.type not in self.ROCKETCHAT_ROOM_TYPES
    ...
    def ensure_room_type_dependencies(self):
        ...
        if (
            ...
            and self.type in self.ROCKETCHAT_ROOM_TYPES
            ...
            def run(self):
                for conference_membership in CosinnusGroupMembership.objects.filter(group=room_self.group):
                    result_group_membership = get_object_or_None(
                        CosinnusGroupMembership, group=room_self.target_result_group, user=conference_membership.user
                    )
    ...
    def non_table_events_qs(self):
        ...
        return (
            self.events.filter(is_break=False)
...
class ParticipationManagement(TranslatableFormsetJsonFieldMixin, models.Model):
    ...
    motivation_questions = models.JSONField(
        default=list, encoder=DjangoJSONEncoder, blank=True, null=True, verbose_name=_('User motivation questions')
    )
    ...
    priority_choice_enabled = models.BooleanField(
        _('Priority choice enabled'), default=settings.COSINNUS_CONFERENCE_PRIORITY_CHOICE_DEFAULT
    )
...
class CosinnusConferenceApplication(models.Model):
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, related_name='user_applications', on_delete=models.CASCADE)
    status = models.PositiveSmallIntegerField(choices=APPLICATION_STATES, default=APPLICATION_SUBMITTED)
    ...
    motivation_answers = models.JSONField(
        default=list, encoder=DjangoJSONEncoder, blank=True, null=True, verbose_name=_('User motivation answers')
    )
    ...
    def first_priorities(self):
        ...
        return [Event.objects.get(id=int(key)) for key, value in self.priorities.items() if value == 1]

    @property
    def second_priorities(self):
        from cosinnus_event.models import Event  # noqa

        return [Event.objects.get(id=int(key)) for key, value in self.priorities.items() if value == 2]
