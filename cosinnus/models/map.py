...
from cosinnus.forms.search import filter_searchqueryset_for_read_access, get_visible_portal_ids
...
def _prepend_url(user, portal=None):
    ...
    return (portal.get_domain() if portal else '') + (
        '' if user.is_authenticated else reverse('cosinnus:user-add') + '?join_msg=1&next='
    )
...
class HaystackMapCard(BaseMapCard):
    ...
    def __init__(self, result, *args, **kwargs):
        if result.portals:
            ...
            portal = (
                current_portal_id
                if ((current_portal_id in displayable_portals) or not displayable_portals)
                else displayable_portals[0]
            )
        else:
...
class HaystackMapResult(BaseMapResult):
    ...
    def __init__(self, result, user=None, *args, **kwargs):
        if result.portals:
            ...
            portal = (
                current_portal_id
                if ((not displayable_portals) or current_portal_id in displayable_portals)
                else displayable_portals[0]
            )
        else:
            portal = result.portal
...
class DetailedBaseGroupMapResult(DetailedMapResult):
    ...
    fields.update({'events': [], 'admins': [], 'organizations': [], 'followed': False, 'starred': False})
    ...
    def __init__(self, haystack_result, obj, user, *args, **kwargs):
        message_url = None
        if (
            not settings.COSINNUS_IS_INTEGRATED_PORTAL
        ...
        kwargs.update(
            {
                ...
                'action_url_1': _prepend_url(user, obj.portal)
                + group_aware_reverse('cosinnus:group-microsite', kwargs={'group': obj}, skip_domain=True)
                + '?join=1',
        ...
        kwargs.update({'events': [HaystackEventMapCard(result) for result in sqs]})
        ...
        kwargs.update({'admins': [HaystackUserMapCard(result) for result in sqs]})
        ...
        if settings.COSINNUS_ORGANIZATIONS_ENABLED:
            ...
            kwargs.update({'organizations': [HaystackOrganizationMapCard(result) for result in sqs]})
...
class DetailedSocietyMapResult(DetailedBaseGroupMapResult):
    ...
    def __init__(self, haystack_result, obj, user, *args, **kwargs):
        ...
        # sqs = filter_searchqueryset_for_read_access(sqs, user)
        sqs = sqs.order_by('title')
        kwargs.update({'projects': [HaystackProjectMapCard(result) for result in sqs]})
...
class DetailedUserMapResult(DetailedMapResult):
    ...
    fields = copy(DetailedMapResult.fields)
    fields.update({'projects': [], 'groups': [], 'extra_html': ''})
    ...
    def __init__(self, haystack_result, obj, user, *args, **kwargs):
        ...
        if (
            ...
            if settings.COSINNUS_ROCKET_ENABLED:
                kwargs.update(
                    {
                        'action_url_1': _prepend_url(user, None)
                        + reverse('cosinnus:message-write', kwargs={'username': obj.user.username}),
                    }
                )
            else:
                kwargs.update(
                    {
                        'action_url_1': _prepend_url(user, None)
                        + reverse('postman:write', kwargs={'recipients': obj.user.username}),
                    }
                )
...
class DetailedEventResult(DetailedMapResult):
    ...
    fields = copy(DetailedMapResult.fields)
    fields.update({'participants': [], 'participant_count': 0, 'followed': False, 'starred': False})
...
class DetailedIdeaMapResult(DetailedMapResult):
    ...
    fields = copy(DetailedMapResult.fields)
    fields.update({'projects': [], 'followed': False, 'starred': False})
...
class DetailedOrganizationMapResult(DetailedMapResult):
    ...
    def __init__(self, haystack_result, obj, user, *args, **kwargs):
        ...
        sqs = filter_searchqueryset_for_read_access(sqs, user)
        sqs = sqs.order_by('title')
        kwargs.update({'groups': [HaystackGroupMapCard(result) for result in sqs]})
