...
class BBBRoom(models.Model):
    ...
    moderator_password = models.CharField(
        max_length=30,
        default=bbb_utils.random_password,
    ...
    voice_bridge = models.PositiveIntegerField(
        help_text=_('pin to enter for telephone participants'),
        verbose_name='dial in PIN',
        default=bbb_utils.random_voice_bridge,
    ...
    last_create_params = models.JSONField(
        verbose_name=_('Last create-call parameters'),
        blank=True,
        null=True,
        default=dict,
        editable=False,
    ...
    def remote_user_count(self):
        ...
        if count is None:
            ...
            cache.set(
                self.PARTICIPANT_COUNT_CACHE_KEY % self.id, count, settings.BBB_ROOM_PARTICIPANTS_CACHE_TIMEOUT_SECONDS
            )
        return count
    ...
    def check_user_can_enter_room(self, user):
        ...
        return bool(
            (user.is_authenticated or type(user) is BBBGuestTokenAnonymousUser) and self.get_password_for_user(user)
        )
    ...
    def create(
        ...
        create_params = cls.build_extra_create_parameters_for_object(
    ...
    def build_extra_create_parameters(self):
        ...
        return self._meta.model.build_extra_create_parameters_for_object(
            self.source_object, meeting_id=self.meeting_id, meeting_name=self.name
        )
    ...
    def build_extra_create_parameters_for_object(cls, source_object, meeting_id=None, meeting_name=None):
        ...
        elif type(source_object) is get_cosinnus_group_model() or issubclass(
    ...
    def build_extra_join_parameters(self, user):
        ...
        if profile and profile.avatar:
            ...
            params.update({'avatarURL': domain + profile.get_avatar_thumbnail_url(size=(800, 800))})
        if profile and profile.language:
            cur_language = translation.get_language()
            params.update({'userdata-bbb_override_default_locale': cur_language})
        return params
    ...
    def _get_bbb_extra_params_for_api_call(cls, api_call_method, source_object):
        ...
        if not source_group.is_premium_ever:
            for premium_preset_name in settings.BBB_PRESET_USER_FORM_FIELDS_PREMIUM_ONLY:
                preset_call_param_dict = list(
                    settings.BBB_PRESET_FORM_FIELD_PARAMS.get(premium_preset_name, {}).values()
                )[0].get(api_call_method, {})
                for blocked_param_name in preset_call_param_dict.keys():
                    portal_default_value = settings.BBB_PARAM_PORTAL_DEFAULTS.get(api_call_method, {}).get(
                        blocked_param_name
                    )
    ...
    def add_guest_link_moderator_only_message_to_params(cls, params, guest_token=None):
        ...
        if guest_token:
            ...
            params['moderatorOnlyMessage'] = (
                f'{message}{guest_link_text} <a href="{guest_url}" target="_blank">{guest_url}</a>'
            )
        return params
    ...
    def get_join_url(self, user):
        ...
        if self.meeting_id and password:
            ...
            return self.bbb_api.join_url(
                self.meeting_id, username, password, extra_parameter_dict=extra_join_parameters
            )
        return ''
    ...
    def get_guest_token(self):
        ...
        if not self.guest_token:
            try:
                ...
                logger.warning(
                    'BBB: Error trying to generate a guest token for BBB room.',
                    extra={'bbb-room-id': self.id, 'exception': str(e)},
                )
    ...
    def get_direct_room_url_for_user(self, user):
        ...
        try:
            ...
            if source_obj and user.is_authenticated:
                BBBRoomVisitStatistics.create_user_visit_for_bbb_room(
                    user, self, group=source_obj.get_group_for_bbb_room()
                )
        except Exception as e:
            if settings.DEBUG:
                raise
...
class BBBRoomVisitStatistics(models.Model):
    ...
    def create_user_visit_for_bbb_room(cls, user, bbb_room, group=None):
        ...
        recent_user_room_visits = cls.objects.filter(bbb_room=bbb_room, user=user, visit_datetime__gte=a_short_time_ago)
