...
class ThumbnailableImageMixin(object):
    ...
    def static_image_url(self, size=None, filename_modifier=None):
        ...
        if not exists(imagepath_local):
            ...
            try:
                ...
                logger.warn(
                    'Invalid Image format on an object',
                    extra={'class': self.__class__.__name__, 'id': getattr(self, 'id')},
                )
                return ''
            except Exception as e:
                logger.warn(
                    'Unexptected Exception while trying to generate a thumbnail!',
                    extra={'class': self.__class__.__name__, 'id': getattr(self, 'id'), 'exception': e},
                )
                return ''
    ...
    def get_media_image_path(self, filename_modifier=None):
        ...
        image_filename = (
            getattr(self, self.image_attr_name).path.split(os.sep)[-1]
            + filename_modifier
            + '.'
            + self.sourcefilename.split('.')[-1]
        )
