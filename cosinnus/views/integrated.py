    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            return HttpResponseNotAllowed(
                'POST', content='Sorry, we could not connect your user account! Please contact an administrator!'
            )
    else:
        raise Http404
    ...
    if request.method == 'POST':
        ...
        if not req.status_code == 200:
            ...
            return HttpResponseBadRequest(
                'Could not create integrated user: Handshake could not be established! Code: %d' % req.status_code
            )
        ...
        cache.set(
            CREATE_INTEGRATED_USER_SESSION_CACHE_KEY % session_key,
            'True',
