...
from cosinnus.utils.group import (
    ...
    get_group_query_filter_for_search_terms,
    prioritize_suggestions_output,
)
...
from cosinnus.utils.urls import get_domain_for_portal, group_aware_reverse, redirect_next_or
...
class CosinnusGroupFormMixin(object):
    ...
    inlines = [
        CosinnusLocationInlineFormset,
        CosinnusGroupGalleryImageInlineFormset,
        CosinnusGroupCallToActionButtonInlineFormset,
    ] + (
        [resolve_class(class_path) for class_path in settings.COSINNUS_GROUP_ADDITIONAL_INLINE_FORMSETS]
        if getattr(settings, 'COSINNUS_GROUP_ADDITIONAL_INLINE_FORMSETS', [])
        else []
    )
    ...
    def dispatch(self, *args, **kwargs):
        ...
        if (
            getattr(settings, 'COSINNUS_LIMIT_PROJECT_AND_GROUP_CREATION_TO_ADMINS', False)
        ...
        society_forbidden = (
            self.form_view == 'add'
            and model_class == CosinnusSociety
            and not getattr(settings, 'COSINNUS_USERS_CAN_CREATE_GROUPS', False)
        )
        conference_forbidden = (
            self.form_view == 'add'
            and model_class == CosinnusConference
            and not getattr(settings, 'COSINNUS_USERS_CAN_CREATE_CONFERENCES', False)
        )
    ...
    def get_form_class(self):
        class CosinnusGroupForm(
            get_form(
                self.group_form_class,
                attachable=False,
    ...
    def get_context_data(self, **kwargs):
        ...
        microsite_public_apps = (
            self.object.get_microsite_public_apps() if self.object else settings.COSINNUS_MICROSITE_DEFAULT_PUBLIC_APPS
        )
    ...
    def post(self, *args, **kwargs):
        ...
        if self.object:
            ...
            deactivated_apps = [app for app in new_apps if app not in self._old_deactivated_apps]
            activated_apps = [app for app in self._old_deactivated_apps if app not in new_apps]
            if activated_apps:
    ...
    def get_success_url(self):
        ...
        if self.object.group_is_conference and self.object.use_conference_applications:
            redirect_url = group_aware_reverse(
                'cosinnus:conference:participation-management', kwargs={'group': self.object}
            )
        else:
...
class GroupCreateView(
    ...
    def forms_valid(self, form, inlines):
        ...
        membership = CosinnusGroupMembership.objects.create(
            user=self.request.user, group=self.object, status=MEMBERSHIP_ADMIN
        )
        ...
        if (
            settings.COSINNUS_IDEAS_ENABLED
            and self.request.POST.get('idea_shortid', None)
            and self.object.type == CosinnusGroup.TYPE_PROJECT
        ):
            ...
            if idea:
                ...
                idea_followers_ids = [pk for pk in idea.get_followed_user_ids() if pk not in [self.request.user.id]]
                idea_followers = get_user_model().objects.filter(id__in=idea_followers_ids)
                cosinnus_notifications.project_created_from_idea.send(
                    sender=self, obj=self.object, user=self.request.user, audience=idea_followers
                )
            else:
                logger.error(
                    'Could not attach an idea to a project on project creatiion because the idea was not found!',
                    extra={'idea_shortid': shortid},
                )
...
class GroupDetailView(
    SamePortalGroupMixin, DetailAjaxableResponseMixin, RequireReadMixin, GroupMembershipMixin, DetailView
):
    ...
    def get_context_data(self, **kwargs):
        ...
        hide_members = (
            settings.COSINNUS_FORUM_HIDE_MEMBER_LIST_FOR_NON_ADMINS
            and self.group.is_mass_invite_group
            and not user_is_superuser
            and not user_is_group_admin
        )
        context.update(
            {
        ...
        if not hide_members:
            ...
            member_ids = [id for id in all_member_ids if id not in admin_ids]
            ...
            is_member_of_this_group = (
                self.request.user.pk in admin_ids
                or self.request.user.pk in member_ids
                or check_user_superuser(self.request.user)
            )
            ...
            membership_objects = self.membership_class.objects.filter(
                user_id__in=membership_object_user_ids, group=self.group
            )
            ...
            if 'invite_token' in self.group.settings:
                ...
                invite_tokens = (
                    token
                    and CosinnusGroupInviteToken.objects.filter(portal=self.group.portal, token__iexact=token)
                    or []
                )
...
class GroupMeetingView(SamePortalGroupMixin, RequireReadMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        self.has_bbb_video = (
            ...
            and self.group.video_conference_type == self.group.BBB_MEETING
        )
        self.has_fairmeeting_video = (
            CosinnusPortal.get_current().video_conference_server
            and self.group.video_conference_type == self.group.FAIRMEETING
        )
        meeting_url = None
...
class GroupListMineView(RequireLoggedInMixin, GroupListView):
    ...
    def get_queryset(self):
        ...
        if not check_user_superuser(self.request.user):
            ...
            my_inactive_groups = my_inactive_groups.filter(
                id__in=model.objects.get_for_user_group_admin_pks(self.request.user, includeInactive=True)
            )
...
class GroupUpdateView(
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'invite_token' in self.group.settings:
            token = self.group.settings.get('invite_token', None)
            invite_tokens = (
                token and CosinnusGroupInviteToken.objects.filter(portal=self.group.portal, token__iexact=token) or []
            )
    ...
    def forms_valid(self, form, inlines):
        if self.group.__class__.__name__ == 'CosinnusConference':
            obj = self.group
            times_changed = self.check_fields_changed(['from_date', 'to_date'], form)
            details_changed = self.check_fields_changed(['name', 'description'], form)
    ...
    def confirm_action(self):
        ...
        try:
            ...
            if m.status == MEMBERSHIP_INVITED_PENDING:
                ...
                messages.success(
                    self.request,
                    _('You had already been invited to "%(team_name)s" and have now been made a member immediately!')
                    % {'team_name': self.object.name},
                )
                signals.user_group_invitation_accepted.send(
                    sender=self,
                    obj=self.object,
                    user=self.request.user,
                    audience=list(get_user_model()._default_manager.filter(id__in=self.object.admins)),
                )
        except self.membership_class.DoesNotExist:
            # auto-join groups will accept immediately
            if self.object.is_autojoin_group:
                self.membership_class.objects.create(
                    user=self.request.user, group=self.object, status=self.membership_status
                )
                messages.success(
                    self.request,
                ...
                self.membership_class.objects.create(
                    user=self.request.user, group=self.object, status=MEMBERSHIP_PENDING
                )
...
class GroupUserInvitationDeclineView(GroupUserWithdrawView):
    ...
    def confirm_action(self):
        try:
            membership = self.membership_class.objects.get(
                user=self.request.user, group=self.object, status=MEMBERSHIP_INVITED_PENDING
            )
            membership.delete()
            signals.user_group_invitation_declined.send(
                sender=self,
                obj=self.object,
                user=self.request.user,
                audience=list(get_user_model()._default_manager.filter(id__in=self.object.admins)),
...
class UserSelectMixin(object):
    ...
    def get(self, *args, **kwargs):
        messages.error(self.request, _('This action is not available directly!'))
        return redirect(
            group_aware_reverse('cosinnus:group-detail', kwargs={'group': kwargs.get('group', '<NOGROUPKWARG>')})
        )
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            if m.status == MEMBERSHIP_PENDING:
                ...
                messages.success(
                    self.request,
                    _('User %(username)s had already requested membership and has now been made a member immediately!')
            ...
            messages.success(
                self.request, _('User %(username)s was successfully invited!') % {'username': user.get_full_name()}
            )
            return ret
    ...
    def form_valid(self, form):
        ...
        if 'invited_groups' in self.group.settings:
            ...
            for group_id in group_ids:
                if group_id not in invited_groups:
        ...
        currently_notified_id_list = list(
        ...
        if newly_invited_users:
            messages.success(
                self.request,
                _('The following users were invited to apply: %s')
                % ', '.join(full_name(user) for user in newly_invited_users),
            )
        else:
            messages.info(
                self.request,
...
class GroupUserUpdateView(AjaxableFormMixin, RequireAdminMixin, UserSelectMixin, UpdateView):
    membership_status = MEMBERSHIP_MEMBER
    ...
    def form_valid(self, form):
        ...
        if user.is_guest:
            ...
            if (
                current_status in [MEMBERSHIP_PENDING, self.membership_status]
                and new_status == MEMBERSHIP_ADMIN
                and not user.id == self.request.user.id
            ):
                cosinnus_notifications.user_group_made_admin.send(
                    sender=self, obj=self.object.group, user=self.request.user, audience=[user]
                )
                # create admin logentry.
                message = _('Made user "%(user)s" admin.') % {'user': user.get_full_name()}
                admin_log_action(self.request.user, self.object.group, message)
            elif (
                current_status == MEMBERSHIP_ADMIN
                and new_status in [MEMBERSHIP_PENDING, self.membership_status]
                and not user.id == self.request.user.id
            ):
                cosinnus_notifications.user_group_admin_demoted.send(
                    sender=self, obj=self.object.group, user=self.request.user, audience=[user]
                )
                # create admin logentry.
    ...
    def form_valid(self, form):
        ...
        if len(group.admins) > 1 or not group.is_admin(user):
            if user != self.request.user or check_user_superuser(self.request.user):
                ...
                messages.error(
                    self.request,
                    _('You cannot remove yourself from a %(team_type)s.')
                    % {'team_type': self.object._meta.verbose_name},
                )
                return HttpResponseRedirect(self.get_success_url())
        else:
        ...
        if current_status == MEMBERSHIP_INVITED_PENDING:
            messages.success(
                self.request,
                _('Your invitation to user "%(username)s" was withdrawn successfully.')
                % {'username': user.get_full_name()},
            )
        if current_status == self.membership_status:
            messages.success(
                self.request, _('User "%(username)s" is no longer a member.') % {'username': user.get_full_name()}
            )
...
class GroupExportView(SamePortalGroupMixin, RequireAdminMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for app, name, label in list(app_registry.items()):
            try:
                url = group_aware_reverse('cosinnus:%s:export' % name, kwargs={'group': self.group})
...
class GroupStartpage(View):
    ...
    def check_redirect_to_microsite(self, request):
        ...
        if not request.user.is_authenticated:
            ...
            if (
                getattr(settings, 'COSINNUS_MICROSITES_DISABLE_ANONYMOUS_ACCESS', False)
        ...
        if request.user.pk not in self.group.members:
...
def group_user_recruit(
    ...
    if not group:
        logger.error(
            'No group found when trying to recruit users!',
    ...
    redirect_url = request.META.get(
        'HTTP_REFERER', group_aware_reverse('cosinnus:group-detail', kwargs={'group': group})
    )

    # do permission checking using has_write_access(request.user, group)
    # TODO: make this available to group.members soon!
    if not is_superuser(user) and user.id not in group.admins:
        logger.error(
            'Permission error when trying to recruit users!',
    ...
    for email in emails:
        ...
        if (
            len(invalid)
            + len(existing_already_members)
            + len(existing_already_invited)
            + len(existing_newly_invited)
            + len(success)
            > MAXIMUM_EMAILS
        ):
            messages.warning(
        ...
        if existing_user:
            ...
            if not membership:
                ...
                logger.error(
                    'Group member recruit: An unreachable else case was reached. Were the membership statuses expanded?'
                )
    ...
    if msg:
        content = mark_safe(
            render_to_string('cosinnus/html_mail/content_snippets/recruit_personal_message.html', {'sender': user})
        )
    ...
    if existing_already_members:
        messages.success(
            request,
            _('Good news! The people with these addresses are already members: %s')
            % ', '.join(existing_already_members),
        )
    if existing_already_invited:
        messages.warning(
            request,
            _('The people with these addresses already have a user account and already have a pending invitation: %s')
            % ', '.join(existing_already_invited),
        )
    ...
    if success:
        messages.success(
            request, _('Success! We are now sending out invitations to these email addresses: %s') % ', '.join(success)
        )
...
def group_user_recruit_delete(request, group, id, invite_class=CosinnusUnregisterdUserGroupInvite):
    ...
    if not group:
        ...
        logger.error(
            'No group found when trying to delete a recruited users!',
            extra={'group_slug': group, 'request': request, 'path': request.path},
        )
        return redirect(reverse('cosinnus:group-list'))
...
def group_assign_reflected_object(request, group):
    ...
    if reflecting_object_content_type not in getattr(settings, 'COSINNUS_REFLECTABLE_OBJECTS', []):
        ...
        for reflectable_group, reflecting in reflect_data['reflectable_groups']:
            ...
            if reflectable_group.id in checked_groups and not reflecting:
                BaseTaggableObjectReflection.objects.create(
                    content_type=ct, object_id=obj.id, group=reflectable_group, creator=request.user
                )
                added_groups.append(reflectable_group)
            # if in reflecting, but not active, get reflecting object, delete it
            if reflecting and reflectable_group.id not in checked_groups:
                BaseTaggableObjectReflection.objects.get(
                    content_type=ct, object_id=obj.id, group=reflectable_group
                ).delete()
    ...
    if added_groups:
        ...
        success_message = (
            force_str(success_message)
            + ' '
            + force_str(
                _('This item is now being shown in these projects/groups: %(group_names)s')
...
class UserGroupMemberInviteSelect2View(RequireReadMixin, Select2View):
    ...
    def get_results(self, request, term, page, context):
        ...
        if direct_email_user:
            ...
            if (
                direct_email_user.id not in self.group.members
                and direct_email_user.id not in self.group.invited_pendings
...
class GroupActivateAppView(SamePortalGroupMixin, AjaxableFormMixin, RequireAdminMixin, View):
    ...
    def post(self, *args, **kwargs):
        ...
        if app and app in app_registry.get_deactivatable_apps() and app in self.group.get_deactivated_apps():
            self.group.deactivated_apps = ','.join(
                list(set([prior_app for prior_app in self.group.get_deactivated_apps() if not prior_app == app]))
            )
    ...
    def invite(self, organization, form):
        try:
            ...
            if m.status == MEMBERSHIP_INVITED_PENDING:
                ...
                # typed_group = ensure_group_type(self.group)
                # typed_group.update_index()
                signals.organization_group_invitation_accepted.send(
                    sender=self, organization=organization, group=self.group
                )
                messages.success(
                    self.request,
                    _('Project/group %(group)s had already been invited and has now been assigned immediately!')
                    % {'name': self.group.name},
                )
                # trigger signal for accepting that user's join request
        except CosinnusOrganizationGroup.DoesNotExist:
            CosinnusOrganizationGroup.objects.create(
                organization=organization, group=self.group, status=MEMBERSHIP_PENDING
            )
