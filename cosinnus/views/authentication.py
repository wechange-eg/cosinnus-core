...
class AdminOnlyOTPTokenValidationView(auth_views.LoginView):
    ...
    template_name = 'cosinnus/authentication/2fa_login_admin.html'
...
class NonAdminOTPTokenValidationView(RequireLoggedInMixin, auth_views.LoginView):
    ...
    def dispatch(self, request, *args, **kwargs):
        user = self.request.user
        if user.is_verified():
            # TODO redirect to proper root
            return redirect('/')
        user.backend = 'cosinnus.backends.EmailAuthBackend'
        return super(NonAdminOTPTokenValidationView, self).dispatch(request, *args, **kwargs)
    ...
    def get_success_url(self):
        url = self.get_redirect_url()
        # TODO: redirect proper start page
        return url or '/map/'


non_admin_otp_token_validation = NonAdminOTPTokenValidationView.as_view()
