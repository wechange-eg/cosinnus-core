...
class ManagedTagsMembersView(AllMembersView):
    ...
    def filter_user_qs(self, user_qs, terms):
        ...
        if self.managed_tag_slug:
            ...
            assigned_profile_ids = profile_assignments_qs.filter(managed_tag__slug=self.managed_tag_slug).values_list(
                'object_id', flat=True
            )
...
class GroupsView(Select2View):
    def get_results(self, request, term, page, context):
        ...
        qs = (
            ...
            .objects.filter(q)
            .filter(portal_id=CosinnusPortal.get_current().id, is_active=True)
        )
