...
from cosinnus.utils.oauth import do_oauth1_receive, do_oauth1_request
...
def callback(request):
    ...
    try:
        ...
        logger.error(
            'Exception during SSO callback, exception was "%s"' % str(e), extra={'trace': traceback.format_exc()}
        )
        messages.error(
            request,
            force_str(
    ...
    try:
        ...
        profile = (
            ...
            .objects.all()
            .extra(where=["settings::json->>'%s' = '%d'" % (SSO_USERPROFILE_FIELD_ID, user_info['id'])])
            .get()
        )
    ...
    profile.language = (
        'de' if user_info.get('locale', None) == 'de_DE' else 'en'
    )  # if "locale" == de_DE -> language german, else english
...
def _get_redirect_url(request):
    ...
    if request.GET.get('next', None) and url_has_allowed_host_and_scheme(
        url=request.GET.get('next'), allowed_hosts=[request.get_host()]
    ):
        return request.GET.get('next')
    if request.session.get('sso-next', None) and url_has_allowed_host_and_scheme(
        url=request.session.get('sso-next'), allowed_hosts=[request.get_host()]
    ):
