...
from cosinnus.views.mixins.group import RequireLoggedInMixin, RequireVerifiedUserMixin, RequireWriteGrouplessMixin
...
class IdeaCreateView(RequireLoggedInMixin, RequireVerifiedUserMixin, CosinnusIdeaFormMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if self.object and self.CREATOR_LIKES_OWN_IDEA:
            ...
            LikeObject.objects.create(
                content_type=content_type, object_id=self.object.id, user=self.request.user, liked=True
            )
...
class IdeaDeleteView(RequireWriteGrouplessMixin, SamePortalGroupMixin, AjaxFormsDeleteViewMixin, DeleteView):
    model = CosinnusIdea
