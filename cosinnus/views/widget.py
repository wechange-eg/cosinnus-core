...
def widget_detail(request, id, offset=0):
    ...
    if (
        not check_user_superuser(request.user)
        and wc.group
        and not (
            check_ug_membership(request.user, wc.group) or wc.group.public or wc.type == WidgetConfig.TYPE_MICROSITE
        )
        or wc.user
        and wc.user_id != request.user.pk
    ):
...
def widget_delete(request, id):
    wc = get_object_or_404(WidgetConfig, id=int(id))
    if wc.group and not check_ug_admin(request.user, wc.group) or wc.user and wc.user_id != request.user.pk:
        return HttpResponseForbidden('Access denied!')
    if request.method == 'POST':
        wc.delete()
...
def widget_edit(request, id, app_name=None, widget_name=None):
    ...
    if app_name and widget_name and (wc.app_name != app_name or wc.widget_name != widget_name):
        ...
        raise Exception(
            'Swapping of widget types is not enabled. \
...
class DashboardWidgetMixin(object):
    ...
    default_widget_order = [
        ...
        'todo.mine',
        'etherpad.latest',
        'cosinnus.group_members',
        'cosinnus.group_projects',
    ]
    ...
    def reorder_equal_widgets(self, widgets):
        ...
        def sort_key(widget):
            try:
                return self.default_widget_order.index(
                    '%s.%s' % (widget.config.app_name, widget.config.widget_name.replace(' ', '_'))
                )
            except ValueError:
                return 999
    ...
    for widget_id, props in list(widgets.items()):
        if 'priority' in props:
            try:
                ...
                if (wc.group and check_object_write_access(wc.group, user)) or (wc.user and wc.user.id == user.id):
