    ...
    def on_error(self, request, *args, **kwargs):
        ...
        if not redirect_url:
            raise ImproperlyConfigured(
                'RequireReadOrRedirectMixin requires "on_error_redirect_url" to be set in the class.'
            )
...
class FilterGroupMixin(object):
    ...
    def get_queryset(self, **kwargs):
        ...
        return qs.filter(**fkwargs).select_related(*select_related)
...
class EndlessPaginationMixin(object):
    ...
    def dispatch(self, request, *args, **kwargs):
        if not self.items_template:
            raise ImproperlyConfigured(
                'You must supply an ``items_template`` template path for the items that are loaded in pagination.'
            )
