...
class MixReflectedObjectsMixin(object):
    ...
    def mix_queryset(self, queryset, model, group=None, user=None):
        ...
        if group is not None:
            reflected_obj_ids = BaseTaggableObjectReflection.objects.filter(group=group, content_type=ct).values_list(
                'object_id', flat=True
            )
        elif user is not None:
            user_group_ids = get_cosinnus_group_model().objects.get_for_user_pks(user)
            reflected_obj_ids = BaseTaggableObjectReflection.objects.filter(
                group_id__in=user_group_ids, content_type=ct
...
class ReflectedObjectSelectMixin(object):
    ...
    def get_reflect_data(self, request, group, obj=None):
        ...
        may_reflect = request.user.is_authenticated and ((group.type == group.TYPE_SOCIETY) or reflect_is_forum)
        if obj is None:
            return {
        ...
        if may_reflect:
            ...
            reflectable_groups = [
                (target_group, target_group.id in reflecting_group_ids)
