...
from cosinnus.utils.urls import group_aware_reverse, iriToUri
...
class FacebookIntegrationViewMixin(object):
    def post_to_facebook(self, userprofile, fb_post_text, urls=[], fb_post_target_id=None, access_token=None):
        ...
        try:
            ...
            if not user_id:
                logger.warning(
                    'Could not post to facebook timeline even though it was requested because of missing fb_userID!',
                    extra={'user-email': userprofile.user.email, 'alternate-post-target': fb_post_target_id},
                )
                return None
            ...
            if not access_token:
                logger.warning(
                    ...
                    extra={
                        ...
                        'user_fbID': user_id,
                        'alternate-post-target': fb_post_target_id,
                    },
                )
                return None
            ...
            if not req.status_code == 200:
                logger.warn(
                    'Facebook posting to timeline failed, request did not return status=200.',
            ...
            logger.warning(
                'Unexpected exception when posting to facebook timeline!',
                extra={
                    'user-email': userprofile.user.email,
                    'user_fbID': user_id,
    ...
    def form_valid(self, form):
        ...
        if form.data.get('facebook_integration_post_to_timeline', None):
            facebook_success = self.post_to_facebook(
                self.request.user.cosinnus_profile, self.object.text, urls=self.object.urls
            )
            if facebook_success is not None:
                if facebook_success:
                    ...
                    self.object.save()
                message_success_addition += ' ' + force_str(
                    _('Your news post was also posted on your Facebook timeline.')
                )
            else:
                messages.warning(
                    self.request,
        ...
        if form.data.get('facebook_integration_post_to_group_page', None) and (
            ...
            facebook_success = self.post_to_facebook(
                self.request.user.cosinnus_profile,
                self.object.text,
                urls=self.object.urls,
                fb_post_target_id=facebook_id,
                access_token=access_token,
            )
            if facebook_success is not None:
                if facebook_success:
                ...
                if group.facebook_page_id and access_token:
                    message_success_addition += ' ' + force_str(
                        _('Your news post was also posted to the Facebook Fan-Page.')
                    )
                elif group.facebook_page_id:
                    message_success_addition += ' ' + force_str(
                        _("Your news post was also posted on the Facebook Fan-Page as a visitor's post.")
                    )
                else:
                    message_success_addition += ' ' + force_str(
                        _('Your news post was also posted in the Facebook Group.')
                    )
            else:
                messages.warning(
                    self.request,
    ...
    def clean(self):
        ...
        if facebook_group_id and facebook_page_id:
            raise forms.ValidationError(
                _('You can only connect to either a Facebook Group or a Fan-Page, but not both!')
            )
        ...
        if facebook_id:
            ...
            try:
                ...
                logger.warn(
                    'Error when trying to retrieve FB group info from Facebook:',
                    extra={'exception': force_str(e), 'url': location_url, 'group_id': facebook_id},
                )
                had_error = True
            if not had_error and not response_info.code == 200:
                logger.warn(
                    'Error when trying to retrieve FB group info from Facebook (non-200 response):',
                    extra={'response_info': force_str(response_info.__dict__), 'group_id': facebook_id},
                )
                had_error = True
            if not had_error:
            ...
            if facebook_page_id:
                obtain_token_result = obtain_facebook_page_access_token_for_user(
                    self.instance, facebook_page_id, self.request.user
                )
...
def confirm_page_admin(request, group_id):
    ...
    return redirect(
        group_aware_reverse('cosinnus:group-edit', kwargs={'group': group}) + '?tab=facebook', permanent=False
    )
...
def obtain_facebook_page_access_token_for_user(group, page_id, user):
    ...
    try:
        ...
        logger.warn(
            'Error when trying to retrieve FB page access-token from Facebook:',
            extra={'exception': force_str(e), 'url': location_url, 'page_id': page_id},
        )
        had_error = True
    if not had_error and not response_info.code == 200:
        logger.warn(
            'Error when trying to retrieve FB page access-token from Facebook (non-200 response):',
            extra={'response_info': force_str(response_info.__dict__), 'page_id': page_id},
        )
        had_error = True
    if not had_error:
...
def save_auth_tokens(request):
    ...
    if 'authResponse' not in request.POST:
    ...
    try:
        ...
        location_url = (
            'https://graph.facebook.com/oauth/access_token?grant_type=fb_exchange_token&client_id=%(app-id)s&client_secret=%(app-secret)s&fb_exchange_token=%(short-lived-token)s'
            % {
        ...
        logger.error(
            ...
            extra={'exception': force_str(e), 'url': location_url},
        )
    ...
    if not response.code == 200:
        logger.error(
            ...
            extra={'response': force_str(response.__dict__)},
        )
    ...
    if (
        'access_token' not in content_auth
    ):  # or not 'expires' in content_auth or not _is_number(content_auth['expires']):
        logger.error(
    ...
    try:
        ...
        logger.warn(
            'Error when trying to retrieve user info from Facebook:',
            extra={'exception': force_str(e), 'url': location_url},
        )
        fb_username = 'error'
    ...
    if not fb_username and not response_info.code == 200:
        logger.warn(
            ...
            extra={'response_info': force_str(response_info.__dict__)},
        )
        fb_username = 'error'
    ...
    return JsonResponse(
        {'status': 'ok', 'username': fb_username, 'user_id': user_id, 'avatar': profile.get_facebook_avatar_url()}
    )
