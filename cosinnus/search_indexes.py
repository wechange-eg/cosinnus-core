...
from cosinnus.utils.user import filter_active_users, filter_portal_users, is_user_active
...
class CosinnusProjectIndex(CosinnusGroupIndexMixin, TagObjectSearchIndex, indexes.Indexable):
    text = TemplateResolveNgramField(
        document=True, use_template=True, template_name='search/indexes/cosinnus/cosinnusgroup_{field_name}.txt'
    )
    rendered = TemplateResolveCharField(
        use_template=True, indexed=False, template_name='search/indexes/cosinnus/cosinnusgroup_{field_name}.txt'
    )
...
class UserProfileIndex(
    LocalCachedIndexMixin, DocumentBoostMixin, StoredDataIndexMixin, TagObjectSearchIndex, indexes.Indexable
):
    text = TemplateResolveNgramField(
        document=True, use_template=True, template_name='search/indexes/cosinnus/userprofile_{field_name}.txt'
    )
    rendered = TemplateResolveCharField(
        use_template=True, indexed=False, template_name='search/indexes/cosinnus/userprofile_{field_name}.txt'
    )
    ...
    def prepare_membership_groups(self, obj):
        ...
        if getattr(settings, 'COSINNUS_USE_V2_DASHBOARD', False):
            group_ids = [pk for pk in group_ids if pk not in get_default_user_group_ids()]
        return group_ids
...
class IdeaSearchIndex(
    LocalCachedIndexMixin, DocumentBoostMixin, TagObjectSearchIndex, StoredDataIndexMixin, indexes.Indexable
):
