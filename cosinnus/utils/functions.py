...
def unique_aware_slugify(
    item, slug_source, slug_field, extra_conflict_check=lambda slug: False, force_redo=False, **kwargs
):
...
def resolve_class(path_to_class):
    ...
    if cls is None:
        raise ImportError('Cannot import class %s from %s' % (klass, path_to_class))
    return cls
...
def resolve_attributes(obj, attr_or_function, default=None, func_args=[]):
    ...
    if not attribute or not hasattr(obj, attribute):
        return (
            resolve_attributes(obj, default, func_args=func_args) if (default and default != attr_or_function) else None
        )
...
def catch_all_and_log(func):
    ...
    def inner_function(*args, **kwargs):
        try:
            ...
            logger.error(
                f'Catch_all_and_log caught exception in {func.__module__}.{func.__name__}().', extra={'exception': e}
            )
