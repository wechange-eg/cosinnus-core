...
from cosinnus.core.middleware.login_ratelimit_middleware import (
    check_user_login_ratelimit,
    register_and_limit_failed_login_attempt,
    reset_user_ratelimit_on_login_success,
)
...
class TwoFactorTokenObtainSerializer(TokenObtainSerializer):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        if settings.COSINNUS_ADMIN_2_FACTOR_AUTH_ENABLED or settings.COSINNUS_USER_2_FACTOR_AUTH_ENABLED:
            self.fields[self.otp_token_field] = serializers.CharField(
                label='OTP Token (optional)', write_only=True, required=False
            )
    ...
    def validate(self, attrs: Dict[str, Any]) -> Dict[Any, Any]:
        ...
        if settings.COSINNUS_ADMIN_2_FACTOR_AUTH_ENABLED or settings.COSINNUS_USER_2_FACTOR_AUTH_ENABLED:
            if user_has_device(self.user) or (
                settings.COSINNUS_ADMIN_2_FACTOR_AUTH_ENABLED and check_user_superuser(self.user)
            ):
