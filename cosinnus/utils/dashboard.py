...
from cosinnus.forms.dashboard import DashboardWidgetForm, EmptyWidgetForm, InfoWidgetForm
...
class DashboardWidget(object):
    ...
    def create(cls, request, group=None, user=None, widget_type=WidgetConfig.TYPE_DASHBOARD):
        ...
        config = WidgetConfig.objects.create(
            app_name=cls.get_app_name(), widget_name=cls.get_widget_name(), group=group, user=user, type=widget_type
        )
    ...
    def title_url(self):
        ...
        if self.config.group:
            return group_aware_reverse('cosinnus:%s:index' % self.app_name, kwargs={'group': self.config.group})
        return ''
...
class GroupMembersWidget(DashboardWidget):
    ...
    def get_data(self, offset=0):
        ...
        qs = (
            ...
            .extra(select={'has_avatar': 'LENGTH(%s.avatar) > 0' % userprofile_table})
...
class GroupProjectsWidget(DashboardWidget):
    ...
    def get_data(self, offset=0):
        ...
        return (
            render_to_string(self.widget_content_template_name, data),
            len(groups),
            True,
        )  # more (create) button always shown
...
def ensure_group_widget(group, app_name, widget_name, widget_type, options):
    ...
    widget_check = WidgetConfig.objects.filter(
        group_id=group.pk,
        app_name=app_name,
        widget_name__in=[widget_name, widget_name.replace('_', ' ')],
        type=widget_type,
    )
