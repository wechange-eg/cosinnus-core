...
class APIRegexURLResolver(object):
    ...
    def __init__(
        ...
        super(APIRegexURLResolver, self).__init__(None, urlconf_name, default_kwargs, app_name, namespace)
            ...
            if self.add_groups:
                if not self.url_group_key:
                    raise ImproperlyConfigured(
                        'Add groups was set to True for API URL matcher, but no group url key was specified!'
                    )
...
def simple_api_pattern_generator(
    version,
    pattern_list,
    app=None,
    add_groups=False,
...
def api_patterns(version, app=None, add_groups=False, *args):
    ...
    if add_groups:
        ...
        for url_key in group_model_registry:
            patterns.extend(
                simple_api_pattern_generator(
                    version, pattern_list, app=app, url_group_key=url_key, add_groups=add_groups
                )
            )
        return patterns
    else:
        return simple_api_pattern_generator(version, pattern_list, app=app, url_group_key=None, add_groups=add_groups)
