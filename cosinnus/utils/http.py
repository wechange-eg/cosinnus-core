...
def add_url_param(url, param_key, param_value):
    """ Given a full URL, this returns the same url with the given GET parameter and its value added. """
    parsed = urlparse(url)
    query = parsed.query
    dic = QueryDict(query)
    dic._mutable = True
...
def remove_url_param(url, param_key, param_value=None):
    ...
    dic = QueryDict(query)
    dic._mutable = True
    if dic.get(param_key, None) and (not param_value or dic.get(param_key, None) == param_value):
        del dic[param_key]
        query = dic.urlencode()
    url = urlunparse((
        parsed.scheme,
