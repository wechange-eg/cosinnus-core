    ...
    if _CosinnusGroup is None:
        ...
        if group_model is None:
            raise ImproperlyConfigured(
                "COSINNUS_GROUP_OBJECT_MODEL refers to model '%s' that has not been installed"
...
def message_group_admins_url(group, group_admins=None):
    ...
    if not group_admins:
        return None
    message_subject = (
        force_str(_('Question about'))
        + ' '
        + force_str(_('Group') if group.type == group.TYPE_SOCIETY else _('Project'))
        + ': '
        + group.name
    )
    return (
        reverse('postman:write', kwargs={'recipients': ','.join([user.username for user in group_admins])})
        + '?subject='
        + urlquote(message_subject)
    )
...
def prioritize_suggestions_output(request, users):
    ...
    current_user_groups_ids = (
        CosinnusGroupMembership.objects.filter(group__is_active=True)
        .filter(status__in=MEMBER_STATUS)
        .filter(user=request.user)
        .values_list('group_id', flat=True)
    )
    users = users.annotate(
        matching_memberships=Count(
            'cosinnus_memberships',
            filter=Q(
                cosinnus_memberships__status__in=MEMBER_STATUS,
                cosinnus_memberships__group_id__in=current_user_groups_ids,
            ),
        )
    )
