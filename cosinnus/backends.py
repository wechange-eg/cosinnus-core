...
class DKIMEmailBackend(EmailBackend):
    def _send(self, email_message):
        ...
        try:
            ...
            signature = dkim.sign(
                message_string, settings.DKIM_SELECTOR, settings.DKIM_DOMAIN, settings.DKIM_PRIVATE_KEY
            )
    ...
    def error_wrapper(self, *args, **kwargs):
        ...
        def do_execute():
            try:
                ...
                logger.error(
                    'An unknown error occured while indexing an object! Exception in extra.',
                    extra={'exception': force_str(e)},
                )
                if settings.DEBUG:
                    raise
