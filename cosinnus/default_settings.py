...
def define_cosinnus_base_settings(project_settings, project_base_path):
    ...
    try:
        ...
        print(
            "Watch out, there is no sentry dsn defined as 'WECHANGE_SENTRY_DSN' in .env, so there is no sentry-support!"
        )
    ...
    try:
        ...
        bbb_str = env('WECHANGE_COSINNUS_BBB_SERVER_CHOICES', default="((0, '(None)'),)")
        COSINNUS_BBB_SERVER_CHOICES = ast.literal_eval(bbb_str)
        bbb_str = env('WECHANGE_COSINNUS_BBB_SERVER_AUTH_AND_SECRET_PAIRS', default='{0: (None, None),}')
    ...
    COSINNUS_CONFERENCES_STREAMING_API_AUTH_PASSWORD = env(
    ...
    try:
        # import pythonic objects from the .env file
        invoice_str = env('WECHANGE_PAYMENTS_INVOICE_BACKEND_AUTH_DATA', default='{}')
        PAYMENTS_INVOICE_BACKEND_AUTH_DATA = ast.literal_eval(invoice_str)
        invoices_str = env('WECHANGE_PAYMENTS_ADDITIONAL_INVOICES_BACKENDS', default='[]')
    ...
    COSINNUS_BASE_PAGE_TITLE_TRANS = (
        project_settings.get('COSINNUS_BASE_PAGE_TITLE_TRANS', None) or project_settings['COSINNUS_PORTAL_NAME']
    )
    ...
    BROKER_URL = 'redis://localhost:6379/44'
