...
class CosinnusUserLoginSerializer(serializers.Serializer):
    ...
    username = serializers.EmailField(
        required=True, help_text='E-Mail of the user account to log in (since we do not accept user names for login)'
    )
...
class CosinnusUserSignupSerializer(
    UserSignupFinalizeMixin, CosinnusUserDynamicFieldsSerializerMixin, serializers.Serializer
):
    ...
    def validate(self, attrs):
        ...
        if (
            ...
            validate_managed_tag_slugs(managed_tag_slugs, settings.COSINNUS_MANAGED_TAGS_USERPROFILE_FORMFIELD_REQUIRED)
    ...
    def create(self, validated_data):
        ...
        if (
            ...
            if validated_data.get('managed_tags', []):
                CosinnusManagedTagAssignment.update_assignments_for_object(
                    user.cosinnus_profile, validated_data.get('managed_tags', [])
                )
    ...
    if pairs_array:
        for pair_dict in pairs_array:
            ...
            if not pair_dict['type'] or pair_dict['type'].lower() not in ACCEPTABLE_TYPES:
                raise ValidationError(
                    f'Contact_infos: A pair ({str(pair_dict)}) had a type not present in [{ACCEPTABLE_TYPES}]!'
                )
...
class CosinnusHybridUserSerializer(TaggitSerializer, CosinnusUserDynamicFieldsSerializerMixin, serializers.Serializer):
    ...
    description = serializers.CharField(
        source='cosinnus_profile.description', required=False, default=None, allow_blank=True
    )
    email = serializers.EmailField(
        required=False, validators=[MaxLengthValidator(220)], read_only=True, help_text='Currently read-only.'
    )
    first_name = serializers.CharField(
        required=False,
        default='',
