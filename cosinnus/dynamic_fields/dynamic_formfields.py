...
class DynamicFieldFormFieldGenerator(object):
    ...
    def get_formfield(
        self,
        dynamic_field_name,
        dynamic_field_options=None,
        dynamic_field_initial=None,
    ...
    def get_formfield_class(self):
        if not self.formfield_class:
            raise Exception('DynamicFieldFormFieldGenerator misconfigured: `formfield_class` is not set!')
...
class LanguageDynamicFieldFormFieldGenerator(_BaseSelect2DynamicFieldFormFieldGenerator):
    def get_formfield_kwargs(self):
        all_languages = [(lang.alpha_2, _(lang.name)) for lang in pycountry.languages if hasattr(lang, 'alpha_2')]
    ...
    def get_formfield_kwargs(self):
        if settings.COSINNUS_MANAGED_TAG_DYNAMIC_USER_FIELD_FILTER_ON_TAG_SLUG:
            data_url = reverse(
                ...
                kwargs={'tag_slug': settings.COSINNUS_MANAGED_TAG_DYNAMIC_USER_FIELD_FILTER_ON_TAG_SLUG},
            )
        else:
...
class FreeChoicesTextDynamicFieldFormFieldGenerator(_BaseSelect2DynamicFieldFormFieldGenerator):
    ...
    def get_formfield_kwargs(self):
        data_url = reverse_lazy(
            'cosinnus:select2:dynamic-freetext-choices', kwargs={'field_name': self._dynamic_field_name}
        )
