...
class DashboardWidgetForm(forms.Form):
    ...
    widget_icon = forms.CharField(
        label=_('Widget icon'), help_text=_('A font-awesome icon for the widget'), required=False
    )
    link_label = forms.CharField(
        label=_('Link label'), help_text=_('Enter a label for the widget-button'), required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['images'] = AttachableWidgetSelect2Field(
            ...
            data_url=group_aware_reverse(
                'cosinnus:attached_object_select2_view', kwargs={'group': group, 'model': 'cosinnus_file.FileEntry'}
            )
            + '?model_as_target=1',
            required=False,
