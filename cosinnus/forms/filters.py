...
class BaseChoiceWidget(forms.Widget):
    ...
    def render(self, name, value, attrs=None, choices=(), renderer=None):
        ...
        render_context = {
            ...
            'widget_id': 'FilterChoiceWidget%s'
            % name,  # self.form_instance.fields[name].creation_counter, # TODO hacky to not error; replace again!
        ...
        for option_value, option_label in chain(self.choices, choices):
            ...
            if isinstance(option_label, (list, tuple)):
                for option in option_label:
                    render_options.append(self.compile_option(name, selected_choices, *option))
            else:
                render_options.append(self.compile_option(name, selected_choices, option_value, option_label))
        return render_options

    def compile_option(self, name, selected_choices, option_value, option_label):
        ...
        try:
            ...
            url = urlencode(data)
        return {'query_string': url, 'label': option_label}
...
class AllObjectsFilter(ChoiceFilter):
    ...
    def field(self):
        ...
        object_qs = getattr(self.model, self.field_name).field.related_model._default_manager.filter(
            id__in=group_object_ids
        )
