...
from multiform import InvalidArgument, MultiModelForm

from cosinnus.fields import UserSelect2MultipleChoiceField
from cosinnus.forms.group import GroupKwargModelFormMixin
from cosinnus.forms.select2 import CommaSeparatedSelect2MultipleChoiceField, TagSelect2Field
...
class BaseTagObjectForm(GroupKwargModelFormMixin, UserKwargModelFormMixin, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if (
            self.data
            and not any([key.startswith('media_tag-') for key in list(self.data.keys())])
            and self.group
            and self.group.media_tag
            and self.group.media_tag.topics
        ):
            self.data._mutable = True
            self.data.setlist('media_tag-topics', self.group.media_tag.topics.split(','))

        if self.group and not self.instance.pk:
            # for new TaggableObjects (not groups), set the default visibility corresponding to the group's public
            # status
        ...
        #   label=_("Persons"), help_text='', required=False, data_url=data_url
        # )
        ...
        if self.group and self.group.media_tag_id:
            ...
            if group_media_tag and group_media_tag is not self.instance:
                ...
                group_data = forms.model_to_dict(group_media_tag, opts.fields, opts.exclude)
        ...
        if self.user and self.instance.pk and self.instance.likers.filter(id=self.user.id).exists():
    ...
    def save(self, commit=True):
        ...
        if commit:
            self.instance.save()
            # For future reference: we skip the call to `save_m2m` here, because the django-multiform `MultiModelForm`
            # already calls it!
            # This should be safe, but maybe in a bug corner case it could cause the `save_m2m` call to be skipped
            # entirely
            # self.save_m2m()
...
def get_tag_object_form():
    ...
    if not issubclass(form_class, forms.ModelForm):
        raise ImproperlyConfigured(
            'COSINNUS_TAG_OBJECT_FORM refers to form '
            "'%s' that does not exist or is not a "
            'ModelForm' % settings.COSINNUS_TAG_OBJECT_FORM
        )
    return form_class
    ...
    class TaggableObjectForm(MultiModelForm):
        ...
        def is_valid(self, *args, **kwargs):
            ...
            if (
                ...
                and any(
                    [
                        (datatag not in self.forms['media_tag'].initial.get('tags', []))
                        for datatag in self.data.getlist('media_tag-tags')
                    ]
                )
            ):
                self.forms['media_tag'].fields['tags'].choices = [
                    (val, val) for val in self.data.getlist('media_tag-tags')
                ]
        ...
        def save(self, commit=True):
            ...
            if commit:
                ...
                # For future reference: we skip the call to `save_m2m` here, because the django-multiform
                # `MultiModelForm` already calls it!
                # This should be safe, but maybe in a bug corner case it could cause the `save_m2m` call to be skipped
                # entirely
                # self.save_m2m()
