        ...
        if target_group and attached_object_registry.get_attachable_to(source_model_id):
            self.fields['attached_objects'] = AttachableObjectSelect2MultipleChoiceField(
                ...
                data_url=group_aware_reverse(
                    'cosinnus:attached_object_select2_view', kwargs={'group': target_group, 'model': source_model_id}
                ),
                required=False,
            )
        ...
        for instance in instances:
            ...
            for attached_obj in self.cleaned_data.get('attached_objects', []):
                ...
                try:
                    ...
                    logger.warning(
                        'Could not set the visibility of an attached object to that of its parent!',
                        extra={'exception': e},
                    )
                    if settings.DEBUG:
                        raise
