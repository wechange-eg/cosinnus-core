...
class CosinnusConferenceRoomForm(GroupKwargModelFormMixin, UserKwargModelFormMixin, forms.ModelForm):
    
    class Meta(BaseTaggableObjectForm.Meta):
        model = CosinnusConferenceRoom
        exclude = ('group', 'slug', 'creator', 'created')
        fields = ['title', 'description', 'type', 'sort_index', 'is_visible',  'allow_user_table_creation',
                  'max_coffeetable_participants', 'target_result_group', 'show_chat']
