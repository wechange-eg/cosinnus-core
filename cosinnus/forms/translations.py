...
from cosinnus.dynamic_fields.dynamic_formfields import EXTRA_FIELD_TYPE_FORMFIELD_GENERATORS as field_generators
...
class TranslatedFieldsFormMixin(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.languages and (model_fields or dynamic_fields):
            field_map = {}
            self.add_model_field_translation_fields(model_fields, field_map)
            self.add_dynamic_fields_translation_fields(dynamic_fields, field_map)

            setattr(self, 'translatable_base_fields', translatable_base_fields)
            setattr(self, 'translatable_field_items', field_map.items())
            setattr(self, 'translatable_fields_languages', [language[0] for language in self.instance.languages])
    ...
    def full_clean(self):
        ...
        if hasattr(self, 'cleaned_data'):
            ...
            self.add_model_translations_to_object(object_translations, form_translations)
            self.add_dynamic_fields_translations_to_object(object_translations, form_translations)
            ...
            for language in self.instance.languages:
                ...
                if field_type in ['CharField', 'TextField']:
                    if field_type == 'CharField':
                        self.fields[field_name] = forms.CharField(label=language[1], required=False)
                    elif field_type == 'TextField':
                        self.fields[field_name] = forms.CharField(
                            widget=forms.Textarea, label=language[1], required=False
                        )
                    field_map[field_name] = self.fields[field_name]

    def get_field_type(self, field):
        return self.instance._meta.get_field(field).get_internal_type()
        ...
        if dynamic_fields and extra_fields:
            for field in dynamic_fields:
                ...
                for language in self.instance.languages:
                    ...
                    formfield = field_generator.get_formfield(field_name, dynamic_field, form=self)
            ...
            if not key == 'dynamic_fields':
                ...
                if translations:
                    for lang in translations.keys():
                        self.initial['{}_translation_{}'.format(key, lang)] = translations.get(lang)
                ...
                if df and translations:
                    ...
                    for lang in languages:
                        ...
                        for field in translation_fields:
                            field_name = '{}_translation_{}'.format(field, lang)
    ...
    def add_model_translations_to_object(self, object_translations, form_data):
        for field in self.instance.get_translateable_fields():
            ...
            for lang in self.instance.languages:
                form_field_name = '{}_translation_{}'.format(field, lang[0])
                if form_field_name in form_data:
                    object_translations.get(field)[lang[0]] = form_data.get(form_field_name)

    def add_dynamic_fields_translations_to_object(self, object_translations, form_data):
        if self.instance.get_translateable_fields():
            ...
            for lang in self.instance.languages:
                for field in self.instance.translatable_dynamic_fields:
                    for lang in self.instance.languages:
                        form_field_name = '{}_translation_{}'.format(field, lang[0])
                        if form_field_name in form_data:
                            if lang[0] not in df_translations:
                                df_translations[lang[0]] = {}
                            df_translations[lang[0]][field] = form_data.get(form_field_name)
