...
def signup_user_to_cleverreach_group_receiver(sender, user, **kwargs):
    ...
    class SignupThread(CosinnusWorkerThread):
        def run(self):
...
def signup_user_to_cleverreach_group(user):
    ...
    cleverreach_group_id = settings.COSINNUS_CLEVERREACH_GROUP_IDS.get(
        language, settings.COSINNUS_CLEVERREACH_GROUP_IDS.get('default', None)
    )
    ...
    try:
        ...
        logger.error(
            'Error when trying to signup a newly registered user to CleverReach (Exception)',
            extra={
                'exception': force_str(e),
                'language': language,
                'cleverreach_form_id': cleverreach_form_id,
                'cleverreach_group_id': cleverreach_group_id,
    ...
    if req.status_code == 200:
        ...
        logger.error(
            'Error when trying to signup a newly registered user to CleverReach (error response on pre-signup check):',
            extra={
                'status': req.status_code,
                'language': language,
                'cleverreach_form_id': cleverreach_form_id,
    ...
    if not req.status_code == 200:
        logger.error(
            'Error when trying to signup a newly registered user to CleverReach (non-200 response):',
            extra={
                'status': req.status_code,
                'language': language,
                'cleverreach_form_id': cleverreach_form_id,
    ...
    if cleverreach_form_id:
        ...
        try:
            # set user inactive
            put_url = (
                '%(base_url)s/groups.json/%(group_id)d/receivers/%(user_email)s/setinactive?token=%(access_token)s'
                % {
                    'base_url': cleverreach_base_url,
                    'group_id': cleverreach_group_id,
                    'user_email': urlquote(user_email),
        ...
        try:
            ...
            logger.error(
                'Error when trying to signup a newly registered user to CleverReach (Exception during Form)',
                extra=extra,
            )
            return
        if not req.status_code == 200:
...
if settings.COSINNUS_CLEVERREACH_AUTO_SIGNUP_ENABLED:
    if (
        not settings.COSINNUS_CLEVERREACH_BASE_URL
        or not settings.COSINNUS_CLEVERREACH_GROUP_IDS
        or not settings.COSINNUS_CLEVERREACH_ACCESS_TOKEN
    ):
        logger.error(
