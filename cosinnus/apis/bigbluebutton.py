...
class BigBlueButtonAPI(object):
    ...
    def __init__(self, source_object=None):
        ...
        if not self.api_auth_url or not self.api_auth_secret:
            raise ImproperlyConfigured(
                'No valid BBB server auth is currently configured or selected as active for this portal!'
            )
    ...
    def _set_current_bbb_server_auth(self, source_object=None):
        ...
        if conference_settings:
            try:
                ...
                recording_api_auth_pair = dict(settings.COSINNUS_BBB_SERVER_AUTH_AND_SECRET_PAIRS).get(
                    conference_settings.bbb_server_choice_recording_api
                )
    ...
    def get_meetings(self):
        ...
        """
        call = 'getMeetings'
        query = urllib.parse.urlencode((('random', 'random'),))
        hashed = self.api_call(query, call)
        url = self.api_auth_url + call + '?' + hashed
        response = requests.get(url)
                ...
                d.append(
                    {
                        ...
                        'info': self.meeting_info(meeting_id, password),
                    }
                )
            return d
        else:
            logger.error(
                'BBB Room error: Server request `getMeetings` was not successful.',
    ...
    def is_running(self, meeting_id):
        ...
        query = urllib.parse.urlencode((('meetingID', meeting_id),))
        hashed = self.api_call(query, call)
        url = self.api_auth_url + call + '?' + hashed
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                if settings.DEBUG:
                    raise
                logger.error(
                    'BBB Room error: Server request `getRecordings` response could not be parsed.',
                    extra={'response_status_code': response.status_code, 'result': response.text, 'exc': e},
                )
                return None
        else:
            logger.error(
                'BBB Room error: Server request `getRecordings` was not successful.',
                extra={'response_status_code': response.status_code, 'result': response.text},
            )
        return None
    ...
    def delete_recorded_meetings(self, record_id):
        ...
        if response.status_code == 200:
            ...
            return success
        logger.error(
            'BBB Room error: Server request `deleteRecordings` was not successful.',
