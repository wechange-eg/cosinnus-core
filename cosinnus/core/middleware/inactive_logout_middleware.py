...
class InactiveLogoutMiddleware:
    ...
    def __call__(self, request, *args, **kwargs):
        if request.user and request.user.is_authenticated:
            last_activity = (
                request.session[INACTIVE_LOGOUT_SESSION_PROPERTY_NAME]
            ...
            if self.logout_time_passed(last_activity):
                ...
                if self.non_api_interface_requested(request.path):
                    ...
                    messages.warning(
                        request, _('You have been automatically logged out after being inactive for some time.')
                    )
                ...
                response = self.get_response(request)
                request.session[INACTIVE_LOGOUT_SESSION_PROPERTY_NAME] = datetime.utcnow().strftime(
                    '%Y-%m-%d %H:%M:%S.%f'
                )
                return response
