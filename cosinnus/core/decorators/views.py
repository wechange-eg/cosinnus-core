...
def redirect_to_error_page(request, view=None, group=None):
    ...
    if group is not None:
        ...
        if not request.user.is_authenticated or request.user.id not in group.members:
...
def get_group_for_request(group_name, request, fail_silently=False):
    ...
    if group_class:
        try:
            ...
            if type(group) is group_class:
                if group.is_active:
                    ...
                    logger.warn(
                        'Cosinnus.core.decorators: Failed to retrieve group because it is inactive!',
                        extra={
                            'team_name': group_name,
                            'url': request.path,
                            'team_type': type(group),
                ...
                logger.warn(
                    'Cosinnus.core.decorators: Failed to retrieve group because its classes didnt match!',
                    extra={
                        'team_name': group_name,
                        'url': request.path,
                        'team_type': type(group),
        ...
        logger.warn(
            'Cosinnus.core.decorators: Failed to retrieve group because no group class was found!',
...
def superuser_required(function):
    ...
    actual_decorator = user_passes_test(lambda u: check_user_superuser(u))
    return actual_decorator(function)
...
def _check_deactivated_app_access(view, group, request):
    ...
    if (
        hasattr(group, 'is_app_deactivated')
        and group.is_app_deactivated(app_name)
...
def require_read_access(group_url_kwarg=None, group_attr=None):
    ...
    def decorator(function):
        ...
        def wrapper(self, request, *args, **kwargs):
            ...
            obj_public = (
                requested_object
                and getattr(requested_object, 'media_tag', None)
        ...
        def wrapper(self, request, *args, **kwargs):
            ...
            if id_url_kwarg is not None:
                ...
                group = get_object_or_404(
                    get_cosinnus_group_model(), id=team_id, portal_id=CosinnusPortal.get_current().id
                )
            elif not group_name:
                return HttpResponseNotFound(_('No team provided'))
            else:
