...
class GroupModelRegistry(DictBaseRegistry):
    ...
    def register(self, url_key, plural_url_key, url_name_prefix, model, form_model, type_index):
        if url_key == plural_url_key:
            raise ImproperlyConfigured(
                'You tried to register a group model with matching url_key and plural_url_key (%s, %s)!'
                % (url_key, plural_url_key)
            )
        for _url_key in self:
            _plural_url_key, _url_name_prefix, _model, _form_model = super(GroupModelRegistry, self).get(
                _url_key, (None, None, None, None)
            )
            if (
                _plural_url_key == plural_url_key
                or _url_key == url_key
                or _plural_url_key == url_key
                or _url_key == plural_url_key
            ):
                raise ImproperlyConfigured(
                    'You tried to register a group model with url_keys (%s, %s) that already existed!'
                    % (url_key, plural_url_key)
                )
            if _model == model:
                raise ImproperlyConfigured(
                    'You tried to register a group model with a model (%s) that already existed!' % (model)
                )
            if _form_model == form_model:
                raise ImproperlyConfigured(
                    'You tried to register a group form model with a model (%s) that already existed!' % (form_model)
                )
    ...
    def get(self, url_key, default=None):
        plural_url_key, url_name_prefix, model, form_model = super(GroupModelRegistry, self).get(
            url_key, (None, None, default, None)
        )
    ...
    def get_by_plural_key(self, plural_url_key, default=None):
        for url_key in self:
            _plural_url_key, url_name_prefix, model, form_model = super(GroupModelRegistry, self).get(
                url_key, (None, None, default, None)
            )
    ...
    def _resolve(self, url_key, plural_url_key, url_name_prefix, model, form_model):
        ...
        if isinstance(model, six.string_types):
            ...
            if cls is None:
                del self[url_key]
                raise ImportError('Cannot import cosinnus renderer %s from %s' % (klass, model))
            else:
                reg_cls = cls
                ret_model = cls
        else:
        ...
        if isinstance(form_model, six.string_types):
            ...
            if cls is None:
                del self[url_key]
                raise ImportError('Cannot import cosinnus renderer %s from %s' % (klass, form_model))
            else:
                reg_form_cls = cls
                ret_form_model = cls
        else:
...
group_model_registry.register(
    ...
    'projects',
    '',
    'cosinnus.models.group_extra.CosinnusProject',
    'cosinnus.forms.group._CosinnusProjectForm',
    0,
)  # CosinnusGroup.TYPE_PROJECT
group_model_registry.register(
    'group',
    'groups',
    'group__',
    'cosinnus.models.group_extra.CosinnusSociety',
    'cosinnus.forms.group._CosinnusSocietyForm',
    1,
)  # CosinnusGroup.TYPE_SOCIETY
if settings.COSINNUS_CONFERENCES_ENABLED:
    group_model_registry.register(
        'conference',
        'conferences',
        'conference__',
        'cosinnus.models.group_extra.CosinnusConference',
        'cosinnus.forms.group._CosinnusConferenceForm',
        2,
    )  # CosinnusGroup.TYPE_SOCIETY
