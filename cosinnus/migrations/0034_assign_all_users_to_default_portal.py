...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        portal = orm.CosinnusPortal.objects.get(id=1)
        for user in orm['auth.User'].objects.all():
            orm.CosinnusPortalMembership.objects.get_or_create(group=portal, user=user, defaults={
                'status': 1,
            })

    def backwards(self, orm):
        orm.CosinnusPortalMembership.objects.all().delete()
