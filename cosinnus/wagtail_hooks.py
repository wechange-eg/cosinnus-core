...
from cosinnus.utils.import_utils import (
    GROUP_IMPORT_PROGRESS_CACHE_KEY,
    GROUP_IMPORT_RESULTS_CACHE_KEY,
    GROUP_IMPORT_RUNNING_CACHE_KEY,
...
if settings.COSINNUS_IMPORT_PROJECTS_PERMITTED:
        ...
        if not import_running and request.method == 'POST':
            if request.POST.get('trigger_new_import', False):
                ...
                messages.success(
                    request,
                    dotrans_(
                        'An import has just finished. Please click the "Start a new import" button to start a new one!'
                    ),
                )
            else:
                ...
                if csv_file_groups and csv_file_users:
                    ...
                    messages.error(
                        request, dotrans_('You did not upload a CSV file or something went wrong during the upload!')
                    )
                else:
                    ...
                    try:
                        ...
                        messages.success(
                            request,
                            dotrans_(
                                'The CSV file was read successfully! You will be notified by email when it completes.'
                            ),
                        )
                        import_running = True
                        ...
                        messages.error(
                            request,
                            dotrans_(
                                'The CSV file you supplied is not formatted in the proper encoding (%s)!' % encoding
                            ),
                        )
                        ...
                        messages.error(
                            request,
                            dotrans_('Another import is currently running! Please wait till that one is finished.'),
                        )
                    except ImproperlyConfigured as e:
                        messages.error(
                            request,
                            dotrans_('A CSV configuration error occured, has the CSV format changed?. Message was: %s')
                            % str(e),
                        )
                        ...
                        logger.warn(
                            'A CSV file uploaded for import encountered an unexpected error! The exception was: "%s"'
                            % str(e),
                            extra={'encoding_used': encoding, 'delimiter_used': delimiter},
                        )
    ...
    def register_import_menu_item():
        return ImportProjectsMenutItem(
            dotrans_('Import'), reverse('import-projects'), classnames='icon icon-plus', order=1005
        )
