...
class UserGuestAccessTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.guest_token = UserGroupGuestAccess.objects.create(group=cls.group, creator=cls.private_group_member, token='testTOKEN')
    ...
    def test_guest_signup(self):
        ...
        signed_in_guest_user = User.objects.get(id=self.client.session.get('_auth_user_id')) # get the user via their auth id
        ...
        self.assertTrue(check_user_can_see_user(signed_in_guest_user, self.private_group_member), 'guest user can see private group member')
    ...
    def test_guest_user_status(self):
        ...
        self.assertNotIn(guest_user, filter_active_users(User.objects.all()), 'a guest user is not listed as active portal user (important so they cannot be invited, messaged, log in on their own, etc)')
