...
class UserNotificationPreference(BaseUserNotificationPreference):
    ...
    group = models.ForeignKey(
        settings.COSINNUS_GROUP_OBJECT_MODEL, related_name='user_notification_preferences', on_delete=models.CASCADE
    )
...
class UserMultiNotificationPreference(BaseUserNotificationPreference):
    ...
    portal = models.ForeignKey(
        ...
        related_name='user_multi_notifications',
        null=False,
        blank=False,
        default=1,
        on_delete=models.CASCADE,
...
class NotificationEvent(models.Model):
    ...
    group = models.ForeignKey(
        settings.COSINNUS_GROUP_OBJECT_MODEL, related_name='notifcation_events', on_delete=models.CASCADE, db_index=True
    )
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
    ...
    def __str__(self):
        return (
            '<NotificationEvent: %(user)s, group: %(group)s, notification_id: %(notification_id)s, date: %(date)s>'
            % {
                'user': self.user,
                'notification_id': self.notification_id,
                'date': str(self.date),
...
class NotificationAlert(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, verbose_name=_('Owner of the alert'), on_delete=models.CASCADE, related_name='+'
    )
    notification_id = models.CharField(_('Notification ID'), max_length=100)
    ...
    portal = models.ForeignKey(
        'cosinnus.CosinnusPortal',
        verbose_name=_('Portal'),
        related_name='notification_alerts',
        null=False,
        blank=False,
        default=1,
        on_delete=models.CASCADE,
    ...
    group = models.ForeignKey(
        ...
        related_name='notifcation_alerts',
        on_delete=models.CASCADE,
        blank=True,
        null=True,
    )
    ...
    type = models.PositiveSmallIntegerField(
        _('Alert State'),
        blank=False,
        default=TYPE_SINGLE_ALERT,
        choices=ALERT_TYPES,
        editable=False,
        help_text='The type of the Alert. Can only morph from single to multi or from single to bundle!',
    )

    target_title = models.CharField(max_length=250, help_text='Cached version of the title of the target object')
    ...
    icon_or_image_url = models.CharField(
        max_length=250,
        blank=True,
        null=True,
    ...
    subtitle = models.CharField(
        max_length=250, blank=True, null=True, help_text='Usually a cached version of the group name, or None'
    )
    subtitle_icon = models.CharField(max_length=250, blank=True, null=True, help_text='fa-icon string or None')
    ...
    multi_user_list = models.JSONField(
        null=True,
        blank=True,
    ...
    def initialize(self, user, target_object, group, action_user, notification_id):
        ...
        self.portal = (
            (self.group and self.group.portal)
...
class SerializedNotificationAlert(DashboardItem):
    ...
    def __init__(self, alert, action_user=None, action_user_profile=None):
        ...
        self['user_icon_or_image_url'] = (
            action_user_profile.get_avatar_thumbnail_url()
            if action_user_profile
            else static('images/jane-doe-small.png')
        )
