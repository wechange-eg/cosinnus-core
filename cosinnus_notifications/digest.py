...
from cosinnus_notifications.notifications import (
    ...
    get_multi_preference_notification_ids,
    get_requires_object_state_check,
    get_superceded_multi_preferences,
    is_notification_multipref,
...
def send_digest_for_current_portal(digest_setting, debug_run_for_user=None, debug_force_show_all=False):
    ...
    if not TIME_DIGEST_START:
        TIME_DIGEST_START = now() - datetime.timedelta(
            days=UserNotificationPreference.SETTINGS_DAYS_DURATIONS[digest_setting]
        )
    TIME_DIGEST_END = now()

    # the main Notification Events QS. anything not in here did not happen in the digest's time span
    timescope_notification_events = NotificationEvent.objects.filter(
        date__gte=TIME_DIGEST_START, date__lt=TIME_DIGEST_END
    )
    if debug_run_for_user:
        ...
        logger.info(
            'Now starting to sending out digests of SETTING=%s in Portal "%s". Data in extra.'
            % (UserNotificationPreference.SETTING_CHOICES[digest_setting][1], portal.slug),
            extra=extra_info,
        )
        if settings.DEBUG:
            print(('>> ', extra_info))
    ...
    for user in users:
        if debug_run_for_user and debug_force_show_all:
            ...
            if (
                ...
                and global_setting == GlobalUserNotificationSetting.SETTING_WEEKLY
            ):
        ...
        try:
            ...
            if global_wanted:
                ...
                exclude_digest_groups = exclude_digest_groups.filter(
                    ...
                    | Q(notification_id=NO_NOTIFICATIONS_ID)
                )
            ...
            for cat_label, cat_notification_ids, cat_icon, cat_url_rev, cat_group_func in categories:
                ...
                if len(categories) > 1:
                    ...
                    category_header_html = render_to_string(
                        'cosinnus/html_mail/summary_group.html', context=header_context
                    )
                ...
                for group in list(set(events.values_list('group', flat=True))):
                    ...
                    for event in group_events:
                        ...
                        if not (
                            current_notification_id in cat_notification_ids
                            or (
                                len(cat_notification_ids) == 0
                                and current_notification_id not in categorized_notification_ids
                            )
                        ):
                            continue
                        if (
                            cat_group_func is not None
                            and hasattr(event, 'group')
                            and event.group
                            and not cat_group_func(event.group)
                        ):
                            continue
                        ...
                        if user is not None and not is_user_active(event.user):
                        ...
                        for other_event in wanted_group_events[:]:
                            if not other_event == this_event:
                                ...
                                if this_event.target_object == other_event.target_object and (
                                    this_event.notification_id == other_event.notification_id
                                    or unprefixed_this_notification_id
                                    in get_superceded_multi_preferences(other_event.notification_id)
                                ):
                    ...
                    if wanted_group_events:
                        ...
                        group_body_html = '\n'.join(
                            [render_digest_item_for_notification_event(event) for event in wanted_group_events]
                        )
            ...
            logger.error(
                'An error occured while doing a digest for a user! Exception was: %s' % force_str(e),
                extra={
                    'exception': e,
    ...
    logger.info(
        'Finished sending out digests of SETTING=%s in Portal "%s". Data in extra.'
        % (UserNotificationPreference.SETTING_CHOICES[digest_setting][1], portal.slug),
        extra=extra_log,
    )
    if settings.DEBUG:
