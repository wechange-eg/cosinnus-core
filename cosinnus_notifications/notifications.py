        ...
        try:
            ...
            if is_html:
                ...
                context = {
                    ...
                    "receiver": receiver,
                    "addressee": (
                        mark_safe(strip_tags(receiver.first_name)) if not type(receiver) is AnonymousUser else None
                    ),
    ...
    def inner_run(self):
        ...
        for receiver in self.audience:
            ...
            if (
                ...
                and receiver.id
                and not receiver.id in self.already_alerted_user_ids
            ):
                try:
...
def render_digest_item_for_notification_event(
    ...
    try:
        ...
        string_variables = {
            ...
            "team_name": (
                escape(notification_event.group["name"])
                if getattr(notification_event, "group", None) is not None
                else "(unknowngroup)"
            ),
            "team_name_short": (
                escape(notification_event.group["name"][:11])
                if getattr(notification_event, "group", None) is not None
                else "(unknowngroup)"
            ),  # first 10 characters given
        }
                ...
                data.update(
                    {
                        ...
                        "action_button_%d_url"
                        % counter: resolve_attributes(obj, options["data_attributes"]["like_button_url"]),
                    }
                )
                counter += 1
                ...
                data.update(
                    {
                        ...
                        "action_button_%d_url"
                        % counter: resolve_attributes(obj, options["data_attributes"]["follow_button_url"]),
                    }
                )
