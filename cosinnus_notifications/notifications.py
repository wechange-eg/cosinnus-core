...
from cosinnus.templatetags.cosinnus_tags import full_name, cosinnus_setting, textfield
...
notifications = {
    ALL_NOTIFICATIONS_ID: {
        "label": _(
            "All notifications"
        ),  # special, hardcoded notification preference has no further settings
        "mail_template": "",
        "subject_template": "",
        "signals": [],
    },
...
MULTI_NOTIFICATION_IDS = {
    ...
    "MULTI_followed_object_notification": getattr(
        settings,
        "COSINNUS_DEFAULT_FOLLOWED_OBJECT_NOTIFICATION_SETTING",
        UserMultiNotificationPreference.SETTING_DAILY,
    ),
}
# the option labels for the multi notifications:
MULTI_NOTIFICATION_LABELS = {
    "MULTI_followed_object_notification": _(
        "For content that I am following, I wish to receive notification emails"
    ),
}
...
NOTIFICATION_REASONS = {
    ...
    "admin": _(
        "You are getting this notification because you are an administrator of this project or group."
    ),
    "portal_admin": _(
        "You are getting this notification because you are an administrator of this portal."
    ),
    "daily_digest": _(
        "You are getting this email because you are subscribed to one or more daily notifications."
    ),
    "weekly_digest": _(
        "You are getting this email because you are subscribed to one or more weekly notifications."
    ),
...
def set_user_group_notifications_special(user, group, all_or_none_or_custom):
    ...
    if not (
        all_or_none_or_custom.startswith("all_")
        or all_or_none_or_custom in ("none", "custom")
    ):
        return

    al = get_object_or_None(
        UserNotificationPreference,
        user=user,
        group=group,
    ...
    if al:
        if all_or_none_or_custom.startswith("all_"):
            ...
            if (
                setting_value
                in list(dict(UserNotificationPreference.SETTING_CHOICES).keys())
                and al.setting != setting_value
            ):
        ...
        if all_or_none_or_custom.startswith("all_"):
            setting_value = int(all_or_none_or_custom.split("_")[1])
            if setting_value not in list(
                dict(UserNotificationPreference.SETTING_CHOICES).keys()
            ):
    ...
    non = get_object_or_None(
        UserNotificationPreference,
        user=user,
        group=group,
        notification_id=NO_NOTIFICATIONS_ID,
    )
    if non:
        ...
        if all_or_none_or_custom == "none":
            UserNotificationPreference.objects.create(
                ...
                group=group,
                notification_id=NO_NOTIFICATIONS_ID,
                setting=UserNotificationPreference.SETTING_NOW,
            )
...
def init_notifications():
            ...
            for signal_id, options in list(notification_module.notifications.items()):
                ...
                for key, default in list(NOTIFICATIONS_DEFAULTS.items()):
                    if options.get(key, None) is None:
                        if (
                            default == REQUIRED_NOTIFICATION_ATTRIBUTE
                            or (
                                options.get("is_html", False)
                                and options.get("can_be_email", True)
                                and default == REQUIRED_NOTIFICATION_ATTRIBUTE_FOR_HTML
                            )
                            or (
                                not options.get("is_html", False)
                                and options.get("can_be_email", True)
                                and default
                                == REQUIRED_NOTIFICATION_ATTRIBUTE_FOR_NON_HTML
                            )
                        ):
                            raise ImproperlyConfigured(
                                'Notification options key "%s" in notification signal "%s" is required!'
                                % (key, signal_id)
                            )
                        options[key] = default
                for datakey, datadefault in list(
                    NOTIFICATIONS_DEFAULTS["data_attributes"].items()
                ):
    ...
    logger.info(
        "Cosinnus_notifications: init complete. Available notification signals: %s"
...
class NotificationsThread(Thread):
    ...
    def _debug_threading(self, state):
        ...
        with sentry_sdk.push_scope() as scope:
            scope.set_tag(
                "notification-object", f"{self.obj._meta.model_name}-{self.obj.id}"
            )
    ...
    def __init__(
        self, sender, user, obj, audience, notification_id, options, first_init=True
    ):
        if first_init:
    ...
    def add_session_frame(self, sender, user, obj, audience, notification_id, options):
        ...
        self.next_session_args.append(
            (sender, user, obj, audience, notification_id, options, False)
        )
    ...
    def is_notification_active(
        self, notification_id, user, group, alternate_settings_compare=[]
    ):
        ...
        try:
            preference = UserNotificationPreference.objects.get(
                user=user, group=group, notification_id=notification_id
            )
            ...
            if notification_id in notifications:
                default_preference_setting = notifications[notification_id].get(
                    "default", 0
                )
    ...
    def check_user_wants_notification(
        self, user, notification_id, obj, notification_moderator_check=False
    ):
        ...
        notification_invite_special = (
            getattr(
                settings,
                "COSINNUS_NOTIFICATIONS_GROUP_INVITATIONS_IGNORE_USER_SETTING",
                False,
            )
            and notification_id == "cosinnus__user_group_invited"
        )
        ...
        if hasattr(obj, "creator"):
            allow_creator_as_audience = False
            if notification_id in notifications:
                allow_creator_as_audience = notifications[notification_id].get(
                    "allow_creator_as_audience", False
                )
        ...
        if not check_object_read_access(obj, user) and not (
            type(obj) is get_cosinnus_group_model()
            or issubclass(obj.__class__, get_cosinnus_group_model())
        ):
            return False
        ...
        if notification_moderator_check:
            if check_user_portal_admin(user) and check_user_portal_moderator(user):
                ...
                if check_object_read_access(obj, AnonymousUser()) or getattr(
                    obj, "cosinnus_always_visible_by_users_moderator_flag", False
                ):
                    return True
        ...
        multi_preference_set = notifications[notification_id].get(
            "multi_preference_set", None
        )
        if multi_preference_set:
            if (
                UserMultiNotificationPreference.get_setting_for_user(
                    user, multi_preference_set
                )
                == UserMultiNotificationPreference.SETTING_NOW
            ):
                return True
            else:
        ...
        if self.is_notification_active(NO_NOTIFICATIONS_ID, user, self.group):
            ...
            user,
            self.group,
            alternate_settings_compare=[
                UserNotificationPreference.SETTING_DAILY,
        ...
        try:
            ...
            if hasattr(receiver, "cosinnus_profile"):  # receiver can be a virtual user
                translation.activate(
                    getattr(
                        receiver.cosinnus_profile, "language", settings.LANGUAGES[0][0]
                    )
                )
                timezone.activate(
                    receiver.cosinnus_profile.timezone.zone
                )  # time zone of receiver
            elif hasattr(
                self.user, "cosinnus_profile"
            ):  # if receiver is a virtual user, set language to sender's
                translation.activate(
                    getattr(
                        self.user.cosinnus_profile, "language", settings.LANGUAGES[0][0]
                    )
                )
                timezone.activate(
                    self.user.cosinnus_profile.timezone.zone
                )  # time zone of sender
            ...
            if self.notification_preference_triggered:
                ...
                pref_arg = ""
                if self.notification_preference_triggered.notification_id not in (
                    NO_NOTIFICATIONS_ID,
                    ALL_NOTIFICATIONS_ID,
                ):
                    pref_arg = "highlight_pref=%d:%s&" % (
                        group_pk,
                        self.notification_preference_triggered.notification_id,
                    )
                url_suffix = "?%shighlight_choice=%d#notif_choice_%d" % (
                    pref_arg,
                    group_pk,
                    group_pk,
                )
            preference_url = "%s%s%s" % (
                domain,
            ...
            if is_html:
                ...
                if reason_key:
                    ...
                    reason = NOTIFICATION_REASONS[
                        self.options.get("notification_reason")
                    ]
                ...
                notification_item_html, data = (
                    render_digest_item_for_notification_event(
                        notification_event, return_data=True
                    )
                )
                ...
                if self.options.get("topic"):
                    topic = mark_safe(
                        self.options.get("topic") % data.get("string_variables")
                    )
                context = {
                    "site": site,
                    "site_name": site.name,
                    ...
                    if type(receiver) is not AnonymousUser
                    else None,
                ...
                subject = _unescape(
                    self.options.get("subject_text") % data.get("string_variables")
                )
                # prefix "[Goupname]" before subject
                team_name_prefix = data.get("string_variables").get(
                    "team_name_short", None
                )
                if team_name_prefix:
    ...
    def create_new_user_alert(self, notification_event, receiver, reason_key=None):
        ...
        create_user_alert(
            notification_event.target_object,
            notification_event.group,
            receiver,
    ...
    def inner_run(self):
        ...
        if hasattr(self.obj, "notification_target_group"):
        ...
        elif type(self.obj) is CosinnusGroup or issubclass(
            self.obj.__class__, CosinnusGroup
        ):
            self.group = self.obj
            ...
            if self.obj.group.type not in self.NOTIFICATION_CONTENT_GROUP_TYPES:
                return
        ...
        setattr(
            notification_event, "_target_object", self.obj
        )  # this helps reduce lookups by local caching the generic foreign key object
        ...
        for receiver in self.audience:
            ...
            if (
                getattr(settings, "COSINNUS_NOTIFICATION_ALERTS_ENABLED", False)
                and options["can_be_alert"]
                and receiver.id
                and receiver.id not in self.already_alerted_user_ids
            ):
                try:
                    alert_reason = self.check_user_wants_alert(
                        receiver, self.notification_id, self.obj
                    )
                    if alert_reason:
                        # create a new NotificationAlert
                        self.create_new_user_alert(
                            notification_event, receiver, reason_key=alert_reason
                        )
                        self.already_alerted_user_ids.append(receiver.id)
                except Exception as e:
                    logger.exception(
                        "An unknown error occured during NotificationAlert check/creation! Exception in extra.",
            ...
            if (
                options["can_be_email"]
                and receiver.email not in self.already_emailed_user_emails
            ):
                if self.check_user_wants_notification(
                    receiver, self.notification_id, self.obj
                ):
        ...
        if self.options["moderatable_content"]:
            portal_admins = get_user_model().objects.filter(
                id__in=CosinnusPortal.get_current().admins
            )
            for admin in portal_admins:
                if admin != self.user:
                    if self.check_user_wants_notification(
                        ...
                        self.obj,
                        notification_moderator_check=True,
                    ):
                        self.send_instant_notification(
                            notification_event, admin, "moderator_alert"
                        )
        ...
        if options["can_be_email"] and self.audience:
            ...
            notifevent = NotificationEvent.objects.create(
                ...
                notification_id=self.notification_id,
                audience=",%s,"
                % ",".join([str(receiver.id) for receiver in self.audience]),
            )
...
def render_digest_item_for_notification_event(
    notification_event,
    return_data=False,
    only_compile_alert_data=False,
    enable_translated_fields=True,
):
    """Renders the HTML of a single notification event for a receiving user"""
    try:
        obj = getattr(
            notification_event, "_target_object", notification_event.target_object
        )
        options = notifications[notification_event.notification_id]

        # Support for `TranslateableFieldsModelMixin
        if enable_translated_fields and hasattr(
            obj, "get_translated_readonly_instance"
        ):
        ...
        object_url = resolve_attributes(
            obj, data_attributes["object_url"], "get_absolute_url"
        )
        ...
        data = {
            ...
            "image_url": resolve_attributes(
                obj, data_attributes["image_url"]
            ),  # TODO: attached image is unused in templates right now
            "alert_image_url": resolve_attributes(
                obj, data_attributes["alert_image_url"]
            ),
            "object_icon": resolve_attributes(obj, data_attributes["object_icon"]),
            "sub_object_icon": resolve_attributes(
                obj, data_attributes["sub_object_icon"]
            ),
        ...
        if getattr(notification_event, "group", None) is not None:
            data.update(
                {
                    "origin_url": notification_event.group.get_absolute_url()
                    + options.get("origin_url_suffix", ""),
                }
            )
                ...
                data.update(
                    {
                        ...
                        "origin_icon_url": get_image_url_for_icon(
                            notification_event.group.get_icon()
                        ),
                        "origin_image_url": portal_url
                        + (
                            notification_event.group.get_avatar_thumbnail_url()
                            or get_image_url_for_icon(
                                notification_event.group.get_icon()
                            )
                        ),
                    }
        ...
        if notification_event.user and notification_event.user.cosinnus_profile:
            data["action_user_url"] = (
                notification_event.user.cosinnus_profile.get_absolute_url()
            )
        ...
        if options["sub_object_icon_url"]:
            ...
            data["sub_object_icon_url"] = get_image_url_for_icon(
                data["sub_object_icon"]
            )
        ...
        elif options.get("show_like_button", False) or options.get(
            "show_follow_button", False
        ):
            obj = getattr(
                notification_event, "_target_object", notification_event.target_object
                ...
                data.update(
                    {
                        ...
                        "action_button_%d_url" % counter: resolve_attributes(
                            obj, options["data_attributes"]["like_button_url"]
                        ),
                    }
                )
                counter += 1
                ...
                data.update(
                    {
                        ...
                        "action_button_%d_url" % counter: resolve_attributes(
                            obj, options["data_attributes"]["follow_button_url"]
                        ),
                    }
                )
        ...
        if not options.get("action_button_text", None) and not data.get(
            "action_button_2_text", None
        ):
            data.update(
                {
                    "show_action_buttons": True,
        ...
        if options.get("action_button_alternate_text", None):
            action_button_alternate_url = options["data_attributes"][
                "action_button_alternate_url"
            ]
            if action_button_alternate_url is None:
                ...
                action_button_alternate_url = resolve_attributes(
                    obj, action_button_alternate_url
                )

            data.update(
                {
                    "show_action_buttons": True,
                    "action_button_alternate_text": options.get(
                        "action_button_alternate_text"
                    ),
        ...
        if len(data["object_name"]) > DIGEST_ITEM_TITLE_MAX_LENGTH:
            data["object_name"] = (
                data["object_name"][: DIGEST_ITEM_TITLE_MAX_LENGTH - 3] + "..."
            )
        ...
        item_html = replace_non_portal_urls(
            item_html, object_url, portal_url=portal_url
        )
        ...
        logger.exception(
            "Error while rendering a digest item for a digest email. Exception in extra.",
            extra={"exception": force_str(e)},
        )
    return ""
...
def notification_receiver(
    sender, user, obj, audience, session_id=None, end_session=False, **kwargs
):
    ...
    audience = [
        aud_user
        for aud_user in audience
        if ((aud_user.is_active or not aud_user.is_authenticated) and aud_user.email)
    ]

    if not session_id:
        # we start this notification thread instantly and alone
        notification_thread = NotificationsThread(
            sender, user, obj, audience, notification_id, options
        )
        ...
        notification_thread.add_session_frame(
            sender, user, obj, audience, notification_id, options
        )
