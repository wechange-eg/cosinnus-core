...
from django.template.defaultfilters import truncatechars_html, truncatewords_html
...
MULTI_NOTIFICATION_IDS = {
    ...
    'MULTI_followed_object_notification': getattr(
        settings, 'COSINNUS_DEFAULT_FOLLOWED_OBJECT_NOTIFICATION_SETTING', UserMultiNotificationPreference.SETTING_DAILY
    ),
}
...
def set_user_group_notifications_special(user, group, all_or_none_or_custom):
    ...
    if al:
        if all_or_none_or_custom.startswith('all_'):
            ...
            if (
                setting_value in list(dict(UserNotificationPreference.SETTING_CHOICES).keys())
        ...
        if all_or_none_or_custom.startswith('all_'):
            setting_value = int(all_or_none_or_custom.split('_')[1])
            if setting_value not in list(dict(UserNotificationPreference.SETTING_CHOICES).keys()):
                setting_value = UserNotificationPreference.SETTING_NOW
            UserNotificationPreference.objects.create(
                user=user, group=group, notification_id=ALL_NOTIFICATIONS_ID, setting=setting_value
            )
...
def init_notifications():
                    ...
                    if options.get(key, None) is None:
                        if (
                            ...
                            or (
                                ...
                                and options.get('can_be_email', True)
                                and default == REQUIRED_NOTIFICATION_ATTRIBUTE_FOR_NON_HTML
                            )
                        ):
    ...
    logger.info(
        'Cosinnus_notifications: init complete. Available notification signals: %s' % list(notifications.keys())
    )
...
class NotificationsThread(CosinnusWorkerThread):
    ...
    def check_user_wants_notification(self, user, notification_id, obj, notification_moderator_check=False):
        ...
        notification_invite_special = (
            getattr(settings, 'COSINNUS_NOTIFICATIONS_GROUP_INVITATIONS_IGNORE_USER_SETTING', False)
        ...
        if not check_object_read_access(obj, user) and not (
            type(obj) is get_cosinnus_group_model() or issubclass(obj.__class__, get_cosinnus_group_model())
        ):
            return False
        ...
        if multi_preference_set:
            if (
                UserMultiNotificationPreference.get_setting_for_user(user, multi_preference_set)
    ...
    def send_instant_notification(self, notification_event, receiver, reason_key=None):
        ...
        try:
            ...
            if hasattr(receiver, 'cosinnus_profile'):  # receiver can be a virtual user
                ...
                timezone.activate(receiver.cosinnus_profile.timezone)  # time zone of receiver
                ...
                timezone.activate(self.user.cosinnus_profile.timezone)  # time zone of sender
            ...
            if is_html:
                ...
                notification_item_html, data = render_digest_item_for_notification_event(
    ...
    def inner_run(self):
        ...
        notification_event = NotificationEvent(
            group=self.group, user=self.user, notification_id=self.notification_id, target_object=self.obj
        )
        setattr(
        ...
        for receiver in self.audience:
            ...
            if options['can_be_email'] and receiver.email not in self.already_emailed_user_emails:
            ...
            for admin in portal_admins:
                if admin != self.user:
                    if self.check_user_wants_notification(
                        admin, self.notification_id, self.obj, notification_moderator_check=True
                    ):
...
def render_digest_item_for_notification_event(
    notification_event, return_data=False, only_compile_alert_data=False, enable_translated_fields=True
):
                ...
                data.update(
                    {
                        ...
                        + (
                            ...
                            or get_image_url_for_icon(notification_event.group.get_icon())
                        ),
                    }
                )
