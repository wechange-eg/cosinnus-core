...
class NotificationsThread(Thread):
    ...
    def check_user_wants_notification(self, user, notification_id, obj, notification_moderator_check=False):
        ...
        if not check_object_read_access(obj, user) and not (isinstance(obj, get_cosinnus_group_model()) or issubclass(obj.__class__, get_cosinnus_group_model())):
            return False
    ...
    def check_user_wants_alert(self, user, notification_id, obj):
        ...
        if isinstance(obj, get_cosinnus_group_model()) or issubclass(obj.__class__, get_cosinnus_group_model()):
            return 'is_group'
        ...
        try:
            ...
            if is_html:
                ...
                context = {
                    ...
                    'receiver': receiver,
                    'addressee': mark_safe(strip_tags(receiver.first_name)) if not isinstance(receiver, AnonymousUser) else None,
    ...
    def inner_run(self):
        ...
        elif isinstance(self.obj, CosinnusGroup) or issubclass(self.obj.__class__, CosinnusGroup):
            self.group = self.obj
        ...
        for receiver in self.audience:
            ...
            if getattr(settings, 'COSINNUS_NOTIFICATION_ALERTS_ENABLED', False) and \
                    options['can_be_alert'] and receiver.id and receiver.id not in self.already_alerted_user_ids:
                try:
