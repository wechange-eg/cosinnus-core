...
def parse_cloud_files_search_response(response_text, path_filter=None, user=None):
    ...
    for search_result in all_responses:
        ...
        if filepath:
            ...
            actual_path = filepath.split(
                f"/dav/files/{settings.COSINNUS_CLOUD_NEXTCLOUD_ADMIN_USERNAME}"
            )[1]
            if path_filter is not None and not path_filter(actual_path):
                continue  # result did not match the path filter
            root_folder_name = urllib.parse.unquote(
                actual_path.split("/")[1]
            )  # starts with '/', so take 2nd item
...
def _get_requesttoken_for_session(session, get_url):
    ...
    if not get_response.status_code == 200:
        raise Exception(
            "Nextcloud admin  login request did not return status code 200! %s"
            % get_response.text
        )
    ...
    try:
        ...
        raise Exception(
            "'data-requesttoken' was not found in <head> tag of Nextcloud admin page!"
        )
    return requesttoken
...
def create_social_login_apps():
    ...
    with requests.Session() as session:
        requesttoken = _get_requesttoken_for_session(
            session, "/settings/admin/sociallogin"
        )
        ...
        if not post_response.status_code == 200:
            raise Exception(
                "Nextcloud admin sociallogin save request did not return status code 200! %s"
                % post_response.text
            )
...
def _make_ocs_call(
    relative_url, post_data={}, headers=HEADERS, session=None, print_to_console=False
):
...
def apply_nextcloud_settings(print_to_console=False):
    ...
    _make_ocs_call(
        ...
        {
            "value": settings.COSINNUS_CLOUD_NEXTCLOUD_SETTINGS[
                "ALLOW_AUTOCOMPLETE_USERS"
            ]
        },
        print_to_console=print_to_console,
    )
