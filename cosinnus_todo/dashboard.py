...
class MyTodosForm(DashboardWidgetForm):
    ...
    amount_subtask = forms.IntegerField(
        label='Amount of Subtasks', initial=2, min_value=0, help_text='0 means unlimited', required=False
    )
...
class MyTodos(DashboardWidget):
    ...
    def title_url(self):
        ...
        if self.config.group and self.request.user.is_authenticated:
            return (
                group_aware_reverse('cosinnus:todo:list', kwargs={'group': self.config.group})
