...
from cosinnus.views.mixins.group import FilterGroupMixin, GroupFormKwargsMixin, RequireReadMixin, RequireWriteMixin
from cosinnus.views.mixins.tagged import EditViewWatchChangesMixin, RecordLastVisitedMixin
from cosinnus.views.mixins.user import UserFormKwargsMixin
from cosinnus_todo import cosinnus_notifications
from cosinnus_todo.filters import TodoFilter
from cosinnus_todo.forms import (
    CommentForm,
    TodoEntryAddForm,
    TodoEntryAssignForm,
    TodoEntryCompleteForm,
    TodoEntryNoFieldForm,
    TodoEntryUpdateForm,
    TodoListForm,
)
...
class InteractiveTodoEntryMixin(object):
    def get_context_data(self, **kwargs):
        ...
        context.update(
            {
                'group_users': filter_active_users(
                    get_user_model().objects.filter(id__in=self.group.members).prefetch_related('cosinnus_profile')
                )
            }
        )
        return context
...
class TodoEntryListView(
    ListAjaxableResponseMixin,
    RequireReadMixin,
    FilterGroupMixin,
    CosinnusFilterMixin,
    InteractiveTodoEntryMixin,
    ListView,
):
    model = TodoEntry
    ...
    def get_queryset(self):
        ...
        qs = super(TodoEntryListView, self).get_queryset(select_related=('assigned_to', 'completed_by', 'todolist'))
        if self.todolist:
...
class TodoListCreateView(
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if list_slug:
            ...
            if default_todolist:
                return redirect(
                    group_aware_reverse(
                        'cosinnus:todo:list-list',
                        kwargs={'group': kwargs.get('group'), 'listslug': default_todolist.slug},
                    )
                )
        ...
        if todo_slug:
            self.todo = get_object_or_404(
                TodoEntry, slug=todo_slug, group__slug=kwargs.get('group'), group__portal=CosinnusPortal.get_current()
            )
        else:
            self.todo = None

        # default filter for todos is completed=False
        if 'is_completed' not in request.GET:
    ...
    def get_queryset(self):
        ...
        # TODO Django>=1.7: change to chained select_related calls
        qs = super(TodoListCreateView, self).get_queryset(select_related=('assigned_to', 'completed_by', 'todolist'))
    ...
    def get_context_data(self, **kwargs):
        ...
        for todolist in self.all_todolists:
            todolist.filtered_items = self.all_todos.filter(
                todolist=todolist
            )  # incomplete_all_todos.filter(todolist=todolist)

        # collect todolists for the move modal
        all_folder_json = []
        for todolist in self.all_todolists:
            all_folder_json.append(
                {'id': todolist.slug, 'parent': '#', 'a_attr': {'target_folder': todolist.id}, 'text': todolist.title}
            )
...
class TodoEntryFormMixin(
    RequireWriteMixin, FilterGroupMixin, GroupFormKwargsMixin, UserFormKwargsMixin, InteractiveTodoEntryMixin
):
    model = TodoEntry

    def get_context_data(self, **kwargs):
        context = super(TodoEntryFormMixin, self).get_context_data(**kwargs)
        context.update({'form_view': self.form_view, 'tags': TodoEntry.objects.filter(group=self.group).tag_names()})
        return context

    def get_success_url(self):
        return group_aware_reverse('cosinnus:todo:entry-detail', kwargs={'group': self.group, 'slug': self.object.slug})
...
class TodoEntryAddView(AjaxableFormMixin, TodoEntryFormMixin, AttachableViewMixin, CreateView):
    ...
    def get_success_url(self):
        return group_aware_reverse(
            'cosinnus:todo:todo-in-list-list',
...
class TodoEntryEditView(
    EditViewWatchChangesMixin, AjaxableFormMixin, TodoEntryFormMixin, AttachableViewMixin, UpdateView
):
    ...
    def on_save_changed_attrs(self, obj, changed_attr_dict):
        ...
        followers_except_creator = [pk for pk in obj.get_followed_user_ids() if pk not in [obj.creator_id]]
        cosinnus_notifications.following_todo_changed.send(
            sender=self,
            user=obj.creator,
            obj=obj,
...
class TodoEntryDeleteView(RequireWriteMixin, FilterGroupMixin, AjaxableFormMixin, DeleteView):
    ...
    def get_success_url(self):
        messages.success(self.request, self.message_success % {'title': self.object.title})
        return group_aware_reverse(
            'cosinnus:todo:list-list', kwargs={'listslug': self.object.todolist.slug, 'group': self.group}
        )
...
class TodoEntryAssignView(TodoEntryEditView):
    ...
    def dispatch(self, request, *args, **kwargs):
        try:
            ...
            messages.error(request, _('You are not allowed to assign this Todo entry.'))
    ...
    def form_valid(self, form):
        ...
        if form.instance.completed_by != form.instance.creator:
            cosinnus_notifications.user_completed_my_todo.send(
                sender=self, user=self.request.user, obj=form.instance, audience=[form.instance.creator]
            )
        # also send notification to all followers except the user who completed this
        followers_except_self = [pk for pk in form.instance.get_followed_user_ids() if pk not in [self.request.user.id]]
        followers_except_self = get_user_model().objects.filter(id__in=followers_except_self)
        cosinnus_notifications.following_todo_completed.send(
            sender=self,
            user=self.request.user,
            obj=form.instance,
            audience=followers_except_self,
    ...
    if request.method == 'POST':
        ...
        if is_completed == 'true':
            ...
            if instance.completed_by != instance.creator:
                ...
                cosinnus_notifications.user_completed_my_todo.send(
                    sender=sender, user=instance.completed_by, obj=instance, audience=[instance.creator]
                )
...
class TodoListListView(ListAjaxableResponseMixin, RequireReadMixin, FilterGroupMixin, ListView):
    model = TodoList
...
class TodoListDetailView(DetailAjaxableResponseMixin, RequireReadMixin, FilterGroupMixin, DetailView):
    model = TodoList
    serializer_class = TodoListSerializer
    ...
    def get_queryset(self):
        ...
        return super(TodoListDetailView, self).get_queryset(select_related=('todos'))
...
class TodoListFormMixin(RequireWriteMixin, FilterGroupMixin, GroupFormKwargsMixin):
    model = TodoList
    ...
    def get_success_url(self):
        return group_aware_reverse('cosinnus:todo:list', kwargs={'group': self.group, 'listslug': self.object.slug})
    ...
    def form_invalid(self, form):
        ...
        messages.error(self.request, self.message_error % {'title': self.object.title})
        return ret
...
class TodoListAddView(AjaxableFormMixin, TodoListFormMixin, CreateView):
    ...
    def get_success_url(self):
        return group_aware_reverse(
            'cosinnus:todo:list-list', kwargs={'group': self.group, 'listslug': self.object.slug}
        )
...
class CommentCreateView(RequireWriteMixin, FilterGroupMixin, AjaxFormsCommentCreateViewMixin, CreateView):
