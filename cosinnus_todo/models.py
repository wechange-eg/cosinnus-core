...
class TodoEntry(LikeableObjectMixin, BaseTaggableObjectModel):
    ...
    due_date = models.DateField(_('Due by'), blank=True, null=True, default=None)

    completed_date = models.DateField(_('Completed on'), blank=True, null=True, default=None)
    ...
    priority = models.PositiveIntegerField(_('Priority'), choices=PRIORITY_CHOICES, default=PRIORITY_MEDIUM)

    note = models.TextField(_('Note'), blank=True, null=True)

    todolist = models.ForeignKey(
        'cosinnus_todo.TodoList',
        verbose_name=_('List'),
        blank=True,
        null=True,
        default=None,
    ...
    def save(self, *args, **kwargs):
        ...
        if created:
            # todo was created
            cosinnus_notifications.todo_created.send(
                sender=self,
                user=self.creator,
                obj=self,
        ...
        if self.__assigned_to != self.assigned_to:
            ...
            if self.assigned_to and self.assigned_to != self.request.user:
                cosinnus_notifications.assigned_todo_to_user.send(
                    sender=self, user=self.request.user, obj=self, audience=[self.assigned_to], session_id=session_id
                )
            # send out a notification to all followers for the change
            followers_except_self = [pk for pk in self.get_followed_user_ids() if pk not in [self.request.user.id]]
            followers_except_self = get_user_model().objects.filter(id__in=followers_except_self)
            cosinnus_notifications.following_todo_assignee_changed.send(
                sender=self,
                user=self.creator,
                obj=self,
    ...
    def on_save_added_tagged_persons(self, set_users):
        ...
        cosinnus_core_notifications.user_tagged_in_object.send(
            sender=self,
            user=self.creator,
            obj=self,
            audience=list(set_users),
            extra={
                'mail_template': 'cosinnus_todo/notifications/user_tagged_in_todo.txt',
                'subject_template': 'cosinnus_todo/notifications/user_tagged_in_todo_subj.txt',
            },
        )
...
class TodoList(models.Model):
    ...
    group = models.ForeignKey(
        settings.COSINNUS_GROUP_OBJECT_MODEL, verbose_name=_('Group'), related_name='+', on_delete=models.CASCADE
    )
...
class Comment(models.Model):
    creator = models.ForeignKey(
        settings.AUTH_USER_MODEL, verbose_name=_('Creator'), on_delete=models.PROTECT, related_name='todo_comments'
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if created:
            ...
            if not self.todo.creator == self.creator:
                cosinnus_notifications.todo_comment_posted.send(
                    sender=self, user=self.creator, obj=self, audience=[self.todo.creator], session_id=session_id
                )

            # message assignee
            if self.todo.assigned_to and not self.todo.assigned_to == self.creator:
                cosinnus_notifications.assigned_todo_comment_posted.send(
                    sender=self, user=self.creator, obj=self, audience=[self.todo.assigned_to], session_id=session_id
                )

            # message all followers of the todo
            followers_except_creator = [
                pk for pk in self.todo.get_followed_user_ids() if pk not in [self.creator_id, self.todo.creator_id]
            ]
            cosinnus_notifications.following_todo_comment_posted.send(
                sender=self,
                user=self.creator,
                obj=self,
            ...
            if self.todo.media_tag and self.todo.media_tag.persons:
                ...
                if len(tagged_users_without_self) > 0:
                    cosinnus_notifications.tagged_todo_comment_posted.send(
                        sender=self,
                        user=self.creator,
                        obj=self,
