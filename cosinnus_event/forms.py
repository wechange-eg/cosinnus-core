...
from cosinnus.forms.conference import CosinnusConferenceSettingsMultiForm, DispatchConferenceSettingsMultiformMixin
...
from cosinnus_event.models import Comment, ConferenceEvent, Event, Suggestion, Vote
...
class _EventForm(
    ...
    def __init__(self, *args, **kwargs):
        ...
        if instance:
            self.fields['suggestion'].queryset = Suggestion.objects.filter(event=instance)
        else:
        ...
        if 'video_conference_type' in self.fields:
            ...
            if settings.COSINNUS_BBB_ENABLE_GROUP_AND_EVENT_BBB_ROOMS:
                if (
                    not settings.COSINNUS_BBB_ENABLE_GROUP_AND_EVENT_BBB_ROOMS_ADMIN_RESTRICTED
                    or self.group.group_can_be_bbb_enabled
                ):
                    custom_choices += [
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'presenters' in self.fields:
            ...
            self.fields['presenters'] = UserSelect2MultipleChoiceField(
                label=_('Presenters'), help_text='', required=False, data_url=data_url
            )
...
class ConferenceEventCoffeeTableForm(
    DispatchConferenceSettingsMultiformMixin,
    get_form(
        _ConferenceEventCoffeeTableForm,
        extra_forms={'conference_settings_assignments': CosinnusConferenceCoffeeTableSettingsMultiForm},
    ),
):
    pass
...
class _ConferenceEventWorkshopForm(_ConferenceEventBaseForm):
    ...
    class Meta(object):
        model = ConferenceEvent
        fields = _ConferenceEventBaseForm.fields + [
            'is_break',
            'from_date',
            'to_date',
            'presenters',
            'presentation_file',
            'max_participants',
            'is_description_visible_on_microsite',
...
class ConferenceEventWorkshopForm(
    DispatchConferenceSettingsMultiformMixin,
    get_form(
        _ConferenceEventWorkshopForm,
...
class ConferenceEventDiscussionForm(
    DispatchConferenceSettingsMultiformMixin,
    get_form(
        _ConferenceEventDiscussionForm,
        extra_forms={'conference_settings_assignments': CosinnusConferenceSettingsMultiForm},
    ),
):
    pass
...
class ConferenceEventStageForm(
    DispatchConferenceSettingsMultiformMixin,
    get_form(
        _ConferenceEventStageForm, extra_forms={'conference_settings_assignments': CosinnusConferenceSettingsMultiForm}
    ),
):
    pass
...
class ConferenceEventLobbyForm(
    DispatchConferenceSettingsMultiformMixin,
    get_form(
        _ConferenceEventLobbyForm, extra_forms={'conference_settings_assignments': CosinnusConferenceSettingsMultiForm}
    ),
):
    pass
