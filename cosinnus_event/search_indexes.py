...
from cosinnus_event.models import Event, EventAttendance, annotate_attendants_count
...
class EventIndex(BaseTaggableObjectIndex, StoredDataIndexMixin, indexes.Indexable):
    ...
    def prepare_participant_count(self, obj):
        ...
        return len(
            self.prepare_participants(obj)
        )  # obj.attendances.filter(state__gt=EventAttendance.ATTENDANCE_NOT_GOING).count()
    ...
    def prepare_participants(self, obj):
        if not hasattr(obj, '_participants'):
            obj._participants = list(
                obj.attendances.filter(state__gt=EventAttendance.ATTENDANCE_NOT_GOING).values_list(
                    'user__id', flat=True
                )
            )
    ...
    def index_queryset(self, using=None):
        ...
        # ony include scheduled events
        qs = qs.exclude(is_hidden_group_proxy=True).filter(conferenceevent__isnull=True, state=Event.STATE_SCHEDULED)
        return qs

    def should_update(self, instance, **kwargs):
        should = super(EventIndex, self).should_update(instance, **kwargs)
        return should and not instance.is_hidden_group_proxy and instance.state == Event.STATE_SCHEDULED
