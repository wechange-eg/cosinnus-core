...
from cosinnus.views.mixins.reflected_objects import (
    MixReflectedObjectsMixin,
...
from cosinnus_event.forms import (
    CommentForm,
    ConferenceEventCoffeeTableForm,
    ConferenceEventDiscussionForm,
    ConferenceEventLobbyForm,
    ConferenceEventStageForm,
    ConferenceEventWorkshopForm,
    DoodleForm,
    EventForm,
    EventNoFieldForm,
    SuggestionForm,
...
from cosinnus_event.models import (
    Comment,
    ConferenceEvent,
    Event,
    EventAttendance,
...
class EventListView(RequireReadMixin, CosinnusFilterMixin, MixReflectedObjectsMixin, FilterGroupMixin, ListView):
    ...
    def get_base_queryset(self):
        ...
        if (
            settings.COSINNUS_EVENT_EVENTS_GROUP_SHOWS_ALL_PUBLIC_EVENTS
            and self.group.slug
            and self.group.slug == getattr(settings, 'NEWW_EVENTS_GROUP_SLUG', None)
        ):
            # mix in public qs
...
class EntryFormMixin(RequireWriteMixin, FilterGroupMixin, GroupFormKwargsMixin, UserFormKwargsMixin):
                ...
                if key.startswith('suggestions-') and key.endswith('_date'):
                    try:
                        ...
                        if key.endswith('from_date'):
                            messages.error(
                                self.request, _('One of the event suggestions times could not be understood!')
                            )
...
class EntryEditView(EditViewWatchChangesMixin, EntryFormMixin, AttachableViewMixin, UpdateWithInlinesView):
    changed_attr_watchlist = [
        ...
        'media_tag.location',
        'url',
        'media_tag.location_lon',
        'media_tag.location_lat',
        'get_attached_objects_hash',
    ]
    ...
    def on_save_changed_attrs(self, obj, changed_attr_dict):
        if not obj.is_hidden_group_proxy:
            ...
            cosinnus_notifications.attending_event_changed.send(
                sender=self,
                user=obj.creator,
                obj=obj,
            ...
            cosinnus_notifications.following_event_changed.send(
                sender=self,
                user=obj.creator,
                obj=obj,
...
class DoodleEditView(EditViewWatchChangesMixin, DoodleFormMixin, AttachableViewMixin, UpdateWithInlinesView):
    ...
    def on_save_changed_attrs(self, obj, changed_attr_dict):
        if not obj.is_hidden_group_proxy:
            # send out a notification to all followers for the change
            followers_except_creator = [pk for pk in obj.get_followed_user_ids() if pk not in [obj.creator_id]]
            cosinnus_notifications.following_doodle_changed.send(
                sender=self,
                user=obj.creator,
                obj=obj,
                audience=get_user_model().objects.filter(id__in=followers_except_creator),
...
class EntryDetailView(
    ...
    def get_context_data(self, **kwargs):
        ...
        user_attendance = (
            None if not user.is_authenticated else get_object_or_None(EventAttendance, user=user, event=event)
        )
        ...
        bbb_room = None
        has_bbb_video = (
            settings.COSINNUS_BBB_ENABLE_GROUP_AND_EVENT_BBB_ROOMS and event.video_conference_type == event.BBB_MEETING
        )
        if has_bbb_video:
...
class DoodleVoteView(RequireReadMixin, RecordLastVisitedMixin, FilterGroupMixin, SingleObjectMixin, FormSetView):
    ...
    def get_initial(self):
        ...
        self.suggestions = self.object.suggestions.order_by('from_date', 'to_date').all()
    ...
    def formset_valid(self, formset):
        for form in formset:
            ...
            if suggestion:
                vote, _created = Vote.objects.get_or_create(suggestion_id=suggestion, voter=self.request.user)
                vote.choice = choice
                vote.save()
        ...
        if not self.object.is_hidden_group_proxy:
            ...
            followers_except_voter = [
                pk for pk in self.object.get_followed_user_ids() if pk not in [self.request.user.id]
            ]
            cosinnus_notifications.following_doodle_voted.send(
                sender=self,
                user=self.request.user,
                obj=self.object,
                audience=get_user_model().objects.filter(id__in=followers_except_voter),
            )
...
def assign_attendance_view(request, group, slug):
    ...
    if not group:
        logger.error(
            'No group found when trying to assign attendance to an event!',
    ...
    if not event:
        logger.error(
            'No event found when trying to assign attendance to an event!',
            extra={'event_slug': slug, 'request': request, 'path': request.path},
        )
    ...
    if not check_object_read_access(event, user):
        logger.warn(
            'Permission error while assigning attendance for an event!',
    ...
    try:
        ...
        logger.error(
            'Exception while assigning attendance for an event!',
            extra={
                'user': user,
                'request': request,
                'path': request.path,
    ...
    return JsonResponse(
        {'error': 'statecouldnotbechanged', 'result_state': -1 if attendance is None else attendance.state}
    )
...
class ConferenceEventFormMixin(
    RequireWriteMixin, FilterGroupMixin, FilterConferenceRoomMixin, GroupFormKwargsMixin, UserFormKwargsMixin
):
    model = ConferenceEvent
    ...
    def get_form_class(self):
        ...
        if klass is None:
            raise ImproperlyConfigured(
                'ConferenceEvent Form type not found for conference room type "%s"' % self.room.type
            )
        return klass
        ...
        context.update(
            {
                ...
                'form_view': self.form_view,
                'room': self.room,
                'event_type_verbose': dict(ConferenceEvent.TYPE_CHOICES)[
                    ConferenceEvent.CONFERENCE_EVENT_TYPE_BY_ROOM_TYPE.get(self.room.type)
                ],
            }
        )
        return context
    ...
    def get_success_url(self):
        if 'create_another' in self.request.POST:
            url = group_aware_reverse(
                'cosinnus:event:conference-event-add', kwargs={'room_slug': self.room.slug, 'group': self.group}
            )
            if self.object.to_date:
                ...
                if start_date:
                    url = '{}?start={}&end={}&starttime={}&endtime={}'.format(
                        url, start_date, to_date, start_time, end_time
                    )
        else:
