...
class EventFormTest(TestCase):
    def setUp(self):
        ...
        self.admin = User.objects.create_superuser(username=self.credential, email=None, password=self.credential)
        self.event = Event.objects.create(group=self.group, creator=self.admin, public=True, title='testevent')
    ...
    def test_has_suggestion(self):
        ...
        Suggestion.objects.create(event=self.event, from_date=now(), to_date=now())
