...
class BBBRoomMixin(models.Model):
    ...
    def get_members_for_bbb_room(self):
        ...
        if group:
            return list(
                get_user_model().objects.filter(id__in=group.members).exclude(email__startswith='__deleted_user__')
            )
        return []
    ...
    def get_moderators_for_bbb_room(self):
        ...
        if group:
            ...
            return list(
                get_user_model().objects.filter(id__in=manager_ids).exclude(email__startswith='__deleted_user__')
            )
        return []
    ...
    def check_and_create_bbb_room(self, threaded=True):
        ...
        if self.can_have_bbb_room() and not self.media_tag.bbb_room:
            ...
            if threaded:
                ...
                class CreateBBBRoomThread(CosinnusWorkerThread):
                    def run(self):
                        create_room()
