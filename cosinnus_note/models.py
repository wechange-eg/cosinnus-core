    ...
    def save(self, *args, **kwargs):
        ...
        if created:
            ...
            if not self.note.creator == self.creator:
                ...
                cosinnus_notifications.note_comment_posted.send(
            ...
            try:
                ...
                followers_except_creator = [
                    pk for pk in self.note.get_followed_user_ids() if pk not in [self.creator_id, self.note.creator_id]
                ]
                cosinnus_notifications.following_note_comment_posted.send(
                    sender=self,
                    user=self.creator,
                    obj=self,
                ...
                commenter_ids = list(
                    set(
                        self.note.comments.exclude(creator__id__in=[self.creator_id, self.note.creator_id]).values_list(
                            'creator', flat=True
                        )
                    )
                )
                commenters = get_user_model().objects.filter(id__in=commenter_ids)
                cosinnus_notifications.note_comment_posted_on_commented_post.send(
                ...
                group_members_without_commenters_ids = [
                    ...
                    if member_id not in [self.creator_id, self.note.creator_id] + commenter_ids
                ]
                all_members = get_user_model().objects.filter(id__in=group_members_without_commenters_ids)
                cosinnus_notifications.note_comment_posted_on_any.send(
                    sender=self,
                    user=self.creator,
                    obj=self,
                ...
                logger.error(
                    'There was an error in the note_comments_commented notification. See extra', extra={'exc': e}
                )
