...
class NoteCreateView(
    ...
    def post(self, request, *args, **kwargs):
        self.referer = self.request.GET.get(
            'next',
            request.META.get('HTTP_REFERER', group_aware_reverse('cosinnus:note:list', kwargs={'group': self.group})),
        )
...
class NoteDetailView(
    ReflectedObjectRedirectNoticeMixin, ReflectedObjectSelectMixin, RequireReadMixin, FilterGroupMixin, DetailView
):
    model = Note
...
class NoteListView(
    ...
    def get_context_data(self, **kwargs):
        kwargs.update({'form': NoteForm(group=self.group)})
...
class NoteEmbedView(
    DipatchGroupURLMixin, MixReflectedObjectsMixin, FilterGroupMixin, PaginationTemplateMixin, ListView
):
    model = Note
    per_page = 10
    ...
    def get_queryset(self, **kwargs):
        ...
        qs = qs.filter(media_tag__visibility=BaseTagObject.VISIBILITY_ALL).prefetch_related(
            'comments__creator__cosinnus_profile', 'attached_objects'
        )
        return qs
...
class NoteUpdateView(
    ...
    def on_save_changed_attrs(self, obj, changed_attr_dict):
        ...
        cosinnus_notifications.following_note_changed.send(
            sender=self,
            user=obj.creator,
            obj=obj,
