...
from cosinnus.views.mixins.reflected_objects import MixReflectedObjectsMixin, ReflectedObjectSelectMixin
from cosinnus_note.forms import NoteForm
from cosinnus_note.models import Note


class DetailedNotesForm(DashboardWidgetForm):
    amount = forms.IntegerField(label='Amount', initial=3, min_value=0, help_text='0 means unlimited', required=False)
...
class DetailedNotes(BaseNotesWidget):
    ...
    def render(self, **kwargs):
        ...
        if group:
            kwargs.update({'form': NoteForm(group=group)})
