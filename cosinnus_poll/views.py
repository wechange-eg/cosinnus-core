...
from cosinnus_poll.forms import CommentForm, OptionForm, PollForm, PollNoFieldForm, VoteForm
from cosinnus_poll.models import Comment, Option, Poll, Vote, current_poll_filter, past_poll_filter
...
class PollListView(RequireReadMixin, FilterGroupMixin, CosinnusFilterMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        running_polls_count = (
            self.queryset.count() if self.poll_view == 'current' else current_poll_filter(self.unfiltered_qs).count()
        )
        past_polls_count = (
            self.queryset.count() if self.poll_view == 'past' else past_poll_filter(self.unfiltered_qs).count()
        )
...
class PollFormMixin(RequireWriteMixin, FilterGroupMixin, GroupFormKwargsMixin, UserFormKwargsMixin):
    form_class = PollForm
    model = Poll
...
class PollEditView(EditViewWatchChangesMixin, PollFormMixin, AttachableViewMixin, UpdateWithInlinesView):
    ...
    def on_save_changed_attrs(self, obj, changed_attr_dict):
        ...
        cosinnus_notifications.following_poll_changed.send(
            sender=self,
            user=obj.creator,
            obj=obj,
...
class PollVoteView(RequireReadMixin, RecordLastVisitedMixin, FilterGroupMixin, SingleObjectMixin, FormSetView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if poll.state == Poll.STATE_VOTING_OPEN and request.user.is_authenticated:
            if check_object_read_access(poll, request.user) and (
                poll.anyone_can_vote or check_ug_membership(request.user, self.group)
            ):
    ...
    def formset_valid(self, formset):
        ...
        if (
            not self.object.multiple_votes
            and not len([True for choice in list(option_choices.values()) if choice == Vote.VOTE_YES]) == 1
        ):
...
class PollCompleteView(RequireWriteMixin, FilterGroupMixin, UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (poll.state, self.mode) not in (
            (Poll.STATE_VOTING_OPEN, 'complete'),
            (Poll.STATE_CLOSED, 'reopen'),
            (Poll.STATE_CLOSED, 'archive'),
        ):
