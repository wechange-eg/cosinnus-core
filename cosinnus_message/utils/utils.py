...
def update_mailboxes():
    ...
    for mailbox in mailboxes:
        try:
            ...
            logger.error(
                'Error trying to collect directreply mails for portal %s' % CosinnusPortal.get_current().slug,
                extra={'exception': force_str(e)},
            )

        logger.info(
            'Successfully gathered directreply messages for portal %s' % CosinnusPortal.get_current().slug,
...
def process_direct_reply_messages(messages=None, no_delete=False):
    ...
    for message in all_messages:
        ...
        if not match or not len(match.groups()) == 2:
            messages_to_delete.append(message)
            logger.info(
                'A directreply-received message did not contain a directreply code.',
                extra={
                    'message-from': message.from_header,
                    'message-text': message.text,
                    'portal-id': CosinnusPortal.get_current().id,
                },
            )
            send_direct_reply_error_mail(
                replier_email, text, _('Your reply could not be matched to an existing message.')
            )
            continue
        ...
        logger.info(
            'A directreply-received message was matched with a directreply code.',
            extra={
                'message-from': message.from_header,
                'message-text': message.text,
                'portal-id': CosinnusPortal.get_current().id,
                'hash': hash,
                'parsed-portal-id': portal_id,
            },
        )
        if not portal_id == CosinnusPortal.get_current().id:
            # message is not for this portal, retain message for other portals
            logger.info(
                'A directreply-received message was matched for a different portal and is being retained.',
                extra={
                    'message-from': message.from_header,
                    'message-text': message.text,
                    'portal-id': CosinnusPortal.get_current().id,
        ...
        try:
            ...
            logger.info(
                'A directreply-received message was matched, but a postman message could not be found with that hash.',
                extra={
                    'message-from': message.from_header,
                    'message-text': message.text,
                    'portal-id': CosinnusPortal.get_current().id,
        ...
        if checkMixin.check_restricted_recipient(postman_message, user):
            send_direct_reply_error_mail(
                replier_email, text, _('Only Administrators may send a message to this project/group!')
            )
            continue
        ...
        if not successfully_replied:
            send_direct_reply_error_mail(
                replier_email, text, _('There was an error when processing your message text!')
            )
            continue
...
def reply_to_postman_message(message, user, text):
    ...
    if form.is_valid():
        ...
        logger.warning(
            'Could not direct-reply to a postman message, because the form was invalid!',
            extra={'form-errors': force_str(form.errors), 'wechange-user-email': user.email, 'text': text},
        )
        return False
...
def write_postman_message(user, sender, subject, text):
    ...
    if form.is_valid():
        ...
        logger.warning(
            'Could not direct-write a postman message, because the form was invalid!',
            extra={'form-errors': force_str(form.errors), 'sender-user-email': user.email, 'text': text},
        )
        return False
...
def send_direct_reply_error_mail(recipient_email, text, reason):
    ...
    logger.warning(
        'Sending out a direct-reply error mail', extra={'recipient': recipient_email, 'reason': reason, 'text': text}
    )
...
def get_rocketchat_mail_notification_setting_from_user_preference(user):
    ...
    from cosinnus_message.rocket_chat import ROCKETCHAT_PREFERENCE_EMAIL_NOTIFICATION_OFF, RocketChatConnection
