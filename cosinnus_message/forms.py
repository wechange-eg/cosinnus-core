...
from cosinnus.utils.user import get_group_select2_pills, get_user_select2_pills
...
class CustomWriteForm(BaseWriteForm):
    ...
    recipients = UserSelect2MultipleChoiceField(
        label=_('Recipients'), help_text='', data_view='postman:user_select2_view'
    )
        ...
        if recipient_list:
            ...
            if use_ids:
                ...
                if (
                    ...
                    and getattr(settings, 'NEWW_FORUM_GROUP_SLUG') in group_tokens
                ):
                    self.restricted_recipient_flag = True
...
class CustomReplyForm(CustomWriteForm):
    ...
    def clean(self):
        if not self.recipient:
            raise forms.ValidationError(_('Undefined recipient.'))
        return super(CustomReplyForm, self).clean()

    def save(self, *args, **kwargs):
        return super(CustomReplyForm, self).save(self.recipient, *args, **kwargs)
