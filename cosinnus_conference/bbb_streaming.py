...
def _get_stream_api_access_token():
    ...
    if response.ok and 'access' in response.json():
        ...
        logger.error(
            'BBB Streaming: Could not get auth access token!',
            extra={'response_code': response, 'response_text': response.text, 'URL': STREAM_API_BASE_URL},
        )
        return None
...
def _stream_api_request(method, sub_url, data=None):
    ...
    if response.ok:
        ...
        logger.error(
            f'BBB Streaming: Request {method}: {sub_url} was not successful!',
            extra={
                'data': data,
...
def create_streamer(name, bbb_url, bbb_secret, meeting_id, stream_url):
    ...
    if response.ok:
        ...
        if 'id' in ret_json.keys():
            ...
            logger.error(
                'BBB Streaming: Create Streamer call was successful but contained no id in data!',
                extra={
                    'streamer_name': name,
                    'ret_json': ret_json,
                },
