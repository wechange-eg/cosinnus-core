    ...
    def get_header_notification(self, obj):
        ...
        if settings.COSINNUS_PREMIUM_CONFERENCES_ENABLED and (check_ug_admin(user, obj) or check_user_superuser(user)):
                ...
                if obj.has_premium_blocks:
                    ...
                    if premium_block_dates:
                        notification_text = (
                            ...
                            + '<br/>'
                            + str(_('Your currently scheduled premium dates are:'))
                            + ' '
                            + ', '.join(premium_block_dates)
                        )
            else:
                notification_text = _(
            ...
            header_notification = {
                ...
                'link_url': render_to_string(
                    'cosinnus/v2/urls/conference_premium_booking_url.html',
                    context={
...
class ConferenceEventSerializer(TranslateableModelSerializer):
    ...
    def get_feed_url(self, obj):
        return group_aware_reverse(
            'cosinnus:team-conference-event-entry', kwargs={'team_id': obj.group.id, 'slug': obj.slug}
        )
