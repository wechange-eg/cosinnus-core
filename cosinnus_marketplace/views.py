...
from cosinnus.core.decorators.views import get_group_for_request, redirect_to_not_logged_in, require_read_access
...
from cosinnus_marketplace.models import Comment, Offer
...
class OfferFormMixin(RequireWriteMixin, FilterGroupMixin, GroupFormKwargsMixin, UserFormKwargsMixin):
...
class OfferEditView(
    EditViewWatchChangesMixin, OfferFormMixin, AttachableViewMixin, MyActiveOfferCountMixin, UpdateWithInlinesView
):
    ...
    def on_save_changed_attrs(self, obj, changed_attr_dict):
        ...
        cosinnus_notifications.following_offer_changed.send(
            sender=self,
            user=obj.creator,
            obj=obj,
...
class OfferDeleteView(RequireWriteMixin, FilterGroupMixin, AjaxFormsDeleteViewMixin, DeleteView):
    model = Offer
...
class OfferDetailView(RequireReadMixin, RecordLastVisitedMixin, FilterGroupMixin, MyActiveOfferCountMixin, DetailView):
    model = Offer
