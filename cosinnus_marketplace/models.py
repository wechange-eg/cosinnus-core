...
class OfferCategory(MultiLanguageFieldMagicMixin, CosinnusBaseCategory):
    category_group = models.ForeignKey(
        OfferCategoryGroup, related_name='categories', null=True, blank=True, on_delete=models.CASCADE
    )
...
class Offer(LikeableObjectMixin, BaseTaggableObjectModel):
    ...
    categories = models.ManyToManyField(
        OfferCategory, verbose_name=_('Offer Category'), related_name='offers', blank=True
    )
    ...
    def get_comment_post_url(self):
        return group_aware_reverse(
            'cosinnus:marketplace:comment', kwargs={'group': self.group, 'offer_slug': self.slug}
        )
...
class Comment(models.Model):
    ...
    def get_edit_url(self):
        return group_aware_reverse(
            'cosinnus:marketplace:comment-update', kwargs={'group': self.offer.group, 'pk': self.pk}
        )

    def get_delete_url(self):
        return group_aware_reverse(
            'cosinnus:marketplace:comment-delete', kwargs={'group': self.offer.group, 'pk': self.pk}
        )
    ...
    def save(self, *args, **kwargs):
        ...
        if created:
            ...
            if not self.offer.creator == self.creator:
                cosinnus_notifications.offer_comment_posted.send(
                    sender=self, user=self.creator, obj=self, audience=[self.offer.creator], session_id=session_id
                )

            # message all followers of the offer
            followers_except_creator = [
                pk for pk in self.offer.get_followed_user_ids() if pk not in [self.creator_id, self.offer.creator_id]
            ]
            cosinnus_notifications.following_offer_comment_posted.send(
                sender=self,
                user=self.creator,
                obj=self,
            ...
            if self.offer.media_tag and self.offer.media_tag.persons:
                ...
                if len(tagged_users_without_self) > 0:
                    cosinnus_notifications.tagged_offer_comment_posted.send(
                        sender=self,
                        user=self.creator,
                        obj=self,
...
def get_categories_grouped(category_qs):
    ...
    category_groups = [
        (cat_group_key_or_misc(grouped_dict[order_key][0], 'name'), grouped_dict[order_key])
        for order_key in sorted(grouped_dict, key=lambda cat_key: cat_key.lower())
    ]
